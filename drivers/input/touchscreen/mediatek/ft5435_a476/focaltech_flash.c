...
void fts_get_upgrade_array(void)
{
	...
	if(chip_id!=0x54)
	{
			printk("yzb_chip_id_err_%s chip_id = %x\n", __func__, chip_id);
			chip_id=0x54;
			chip_id_err =1;
	}
	printk("yzb_tp_%s chip_id = %x\n", __func__, chip_id);
...
{	
		...
		for (i = 0; i < FTS_PACKET_LENGTH; i++) 
	{
		...
		for(i = 0;i < 30;i++)
		{
			...
			printk("[FTS][%s] reg_val[0] haha = %x reg_val[1] = %x \n", __func__, reg_val[0], reg_val[1]);
			msleep(1);			
		}
		printk("leave");
	}
	...
	//		printk("[FTS][%s]jj reg_val[0] = %x reg_val[1] = %x \n", __func__, reg_val[0], reg_val[1]);
			msleep(1);
	...
	FTS_DBG("Step 6: read out checksum\n");
	auc_i2c_write_buf[0] = 0xcc;
	fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val_i2c, 1);
	auc_i2c_write_buf[0] = 0x64;
	fts_i2c_write(client, auc_i2c_write_buf, 1); 
	msleep(300);
//	printk("yzb--1111111\n");
	...
	if (reg_val[0] != bt_ecc) 
	{
		dev_err(&client->dev, "[FTS]--k=%d, ecc error! fw_ecc=%02x flash_ecc=%02x i2c_ecc=%2X\n",
					k,reg_val[0],
...
int  fts_8606_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	//选\D4\F1Flash\D0秃牛\AC\C3\FC\C1睿?W 05 Mode\A1\A3 Mode\A3\BAFT5003\A3\BA0x81\A3\AC Winbond\A3\BA0x80
	//ucAddr = 0x05;
	//ucMode = 0x80;
		...
		if (i_ret != 0)
{
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x86))
	{
		...
		if (i_ret != 0)
		{
			printk(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
		}
		else 
		{
			printk("yzb---OK\n");
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	//	) 
	if (chip_id_err||(uc_tp_fm_ver < uc_host_fm_ver)) //avoid auto upgrade fail, it will lead to read/write reg fail
//	if(1)                 //yzb
	{
		printk("yzb__in updtate__tp\n");
		msleep(100);
		...
		if (i_ret == 0)	
		{
			msleep(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			printk(&client->dev, "[FTS] upgrade to new version 0x%x\n",uc_host_fm_ver);
		} 
		else
		{
