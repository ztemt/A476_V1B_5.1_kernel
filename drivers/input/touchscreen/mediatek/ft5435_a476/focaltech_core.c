...
#define __MSG_DMA_MODE__
	...
	{
		g_dma_buff_va = (u8 *)dma_alloc_coherent(&fts_i2c_client->dev, 4096, &g_dma_buff_pa, GFP_KERNEL);//DMA size 4096 for customer
	    	if(!g_dma_buff_va)
		{
...
static struct i2c_board_info __initdata fts_i2c_tpd={ I2C_BOARD_INFO("fts", (0x7C>>1))};
...
static int fts_report_value(struct ts_event *data)
 {
	...
	*/			
	...
	//	 input_mt_slot(tpd->dev, data->au8_finger_id[i]);
 		input_report_abs(tpd->dev, ABS_MT_TRACKING_ID, data->au8_finger_id[i]);
...
***********************************************************************/
 static int touch_event_handler(void *unused)
 {
	...
	do
	{
		...
		#endif
        fts_updateinfo_curr.TPD_MAX_POINTS =10;                        
		#ifdef MT_PROTOCOL_B
		{
...
***********************************************************************/
 ...
 {	 
	...
	msleep(200);
 	...
 	err = i2c_smbus_read_i2c_block_data(fts_i2c_client, 0x00, 1, &data);//if auto upgrade fail, it will not read right value next upgrade.
	...
	{
		TPD_DMESG("a476_ft5435 I2C transfer error, line: %d\n", __LINE__);
...
***********************************************************************/
 static void tpd_suspend( struct early_suspend *h )
 {
	...
	*/
	...
	#else
	printk("yzb---1111\n");
	for(i = 0; i< TPD_MAX_POINTS_10-1; i++) {
	input_report_key(tpd->dev, BTN_TOUCH, 0);
	input_mt_sync(tpd->dev);
	}
	input_sync(tpd->dev);
