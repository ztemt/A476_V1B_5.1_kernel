...
#include "t4ka3mipiraw_Sensor_cos.h"

extern int iReadReg(u16 a_u2Addr , u8 * a_puBuff , u16 i2cId);
extern int iWriteReg(u16 a_u2Addr , u32 a_u4Data , u32 a_u4Bytes , u16 i2cId);

#define PFX "t4ka3cos_4lane_camera_sensor"
...
#define T4KA3COS_OTP_PSEL 0x4A02
#define T4KA3COS_OTP_CTRL 0x4A00

#define T4KA3COS_OTP_DATA_BEGIN_ADDR 0x4A04
#define T4KA3COS_OTP_DATA_END_ADDR   0x4A43
static uint16_t T4KA3COS_OTP_DATA[T4KA3COS_OTP_DATA_END_ADDR - T4KA3COS_OTP_DATA_BEGIN_ADDR + 1] = {0x00};
static uint16_t T4KA3COS_OTP_DATA_BACKUP[T4KA3COS_OTP_DATA_END_ADDR - T4KA3COS_OTP_DATA_BEGIN_ADDR + 1] = {0x00};

//static kal_uint16 t4ka3cos_r_golden_value=0x54; //golden module  AWB R value
//static kal_uint16 t4ka3cos_g_golden_value=0x92; //golden module  AWB G value: (Gr+Gb)/2
//static kal_uint16 t4ka3cos_b_golden_value=0x5A; //golden module  AWB B value, 

static kal_uint16 t4ka3cos_r_golden_value=0x5e; //golden module  AWB R value
static kal_uint16 t4ka3cos_g_golden_value=0x93; //golden module  AWB G value: (Gr+Gb)/2
static kal_uint16 t4ka3cos_b_golden_value=0x5b; //golden module  AWB B value, 

static kal_uint16 t4ka3cos_r_cur_value=0; //current module  AWB R value
static kal_uint16 t4ka3cos_g_cur_value=0; //current module  AWB G value: (Gr+Gb)/2
static kal_uint16 t4ka3cos_b_cur_value=0; //current module  AWB B value
static kal_uint16 t4ka3cos_r_gain=0; 
static kal_uint16 t4ka3cos_b_gain=0; 


static imgsensor_info_struct imgsensor_info = {
    .sensor_id = 0x1490,        //record sensor id defined in Kd_imgsensor.h
...
static int32_t t4ka3cos_otp_read_data(uint16_t* otp_data)
{
    kal_uint16 i = 0;
	
    for (i = 0; i <= (T4KA3COS_OTP_DATA_END_ADDR - T4KA3COS_OTP_DATA_BEGIN_ADDR); i++)
	{
        otp_data[i]=read_cmos_sensor(T4KA3COS_OTP_DATA_BEGIN_ADDR+i);
...
// T4KA3COS Initial 3264x2448_30FPS_MIPI_4LANE(Binning)
...
static void preview_setting(void)
{
    ...
    mdelay(50);
   //t4ka3cos preview size 1632x1224 30fps
...
{
	 //t4ka3cos capture size 3264x2448 30fps
	  write_cmos_sensor(0x0100,0x00);//standby mode
    mdelay(10);
	 write_cmos_sensor(0x034C,0x0C);//X_OUTPUT_SIZE[15:8];
	 write_cmos_sensor(0x034D,0xC0);//X_OUTPUT_SIZE[7:0];
...
static void normal_video_setting(kal_uint16 currefps)
{
	 ...
	 //t4ka3cos normal video size 1632x1224 30fps
	  write_cmos_sensor(0x0100,0x00);//standby mode
    mdelay(10);
	 write_cmos_sensor(0x034C,0x06);//X_OUTPUT_SIZE[15:8];
	 write_cmos_sensor(0x034D,0x60);//X_OUTPUT_SIZE[7:0];
...
{
	 //t4ka3cos hs video size 1632x1224 30fps
	  write_cmos_sensor(0x0100,0x00);//standby mode
    mdelay(10);
	 write_cmos_sensor(0x034C,0x06);//X_OUTPUT_SIZE[15:8];
	 write_cmos_sensor(0x034D,0x60);//X_OUTPUT_SIZE[7:0];
...
{
	 //t4ka3cos slim video size 1632x1224 30fps
	   write_cmos_sensor(0x0100,0x00);//standby mode
    mdelay(10);
	 write_cmos_sensor(0x034C,0x06);//X_OUTPUT_SIZE[15:8];
	 write_cmos_sensor(0x034D,0x60);//X_OUTPUT_SIZE[7:0];
...
static void t4ka3cos_otp_read_enable(kal_uint8 enable)
{
   if(enable)
   	write_cmos_sensor(T4KA3COS_OTP_CTRL,0x01);
   else
   	write_cmos_sensor(T4KA3COS_OTP_CTRL,0x00);
}

static void t4ka3cos_otp_set_page(kal_uint16 page)
{
   write_cmos_sensor(T4KA3COS_OTP_PSEL,page);
}

static void t4ka3cos_otp_start_read()
{
  write_cmos_sensor(T4KA3COS_OTP_CTRL,(read_cmos_sensor(T4KA3COS_OTP_CTRL) | 0x80));
}

static void t4ka3cos_otp_checkMag()
{
  
  kal_uint8 checkMag;
  
  t4ka3cos_otp_set_page(0x0f);
  t4ka3cos_otp_start_read();
...
static int t4ka3cos_otp_update_lsc()
{
	 kal_uint8 lsc_flag;
	 kal_uint8 i;	 
   ...
   {
		t4ka3cos_otp_read_enable(1);
		
		t4ka3cos_otp_set_page(3);
		t4ka3cos_otp_start_read();
		for(i=0;i<64;i++)  
		{
			lsc_buff1[i] = read_cmos_sensor(T4KA3COS_OTP_DATA_BEGIN_ADDR+i);
		}
		
		t4ka3cos_otp_set_page(4);
		t4ka3cos_otp_start_read();
		for(i=0;i<64;i++)  
		{
			lsc_buff2[i] = read_cmos_sensor(T4KA3COS_OTP_DATA_BEGIN_ADDR+i);
		}
		
		t4ka3cos_otp_set_page(5);
		t4ka3cos_otp_start_read();
		for(i=0;i<64;i++)  
		{
			lsc_buff3[i] = read_cmos_sensor(T4KA3COS_OTP_DATA_BEGIN_ADDR+i);
		}

		t4ka3cos_otp_set_page(6);
		t4ka3cos_otp_start_read();
		for(i=0;i<64;i++)  
		{
			lsc_buff4[i] = read_cmos_sensor(T4KA3COS_OTP_DATA_BEGIN_ADDR+i);
		}
...
static void t4ka3cos_otp_update_awb()
{  
  int i , j,sum=0,flag;
  ...
  flag = read_cmos_sensor(0x4A08);
	mDELAY(2);  
  t4ka3cos_otp_set_page(0x02);
  t4ka3cos_otp_start_read();
 // t4ka3cos_otp_read_data(OTP_data); //For printf all AWB inf
  		if(flag == 0x00)
  			{
				printk("AWB is empty\n");
			...
			{
				printk("first OTP\n");
				t4ka3cos_r_cur_value=((read_cmos_sensor(0x4A04) << 8) | read_cmos_sensor(0x4A05));
  				t4ka3cos_g_cur_value=((read_cmos_sensor(0x4A06) << 8) | read_cmos_sensor(0x4A07));
  				t4ka3cos_b_cur_value=((read_cmos_sensor(0x4A08) << 8) | read_cmos_sensor(0x4A09));
				checksumpage = read_cmos_sensor(0x4A0A);
  				printk("checksumpage=0x%x\n",checksumpage);
				  //checking awb check sum
   			//	for( i=0; i<6;  i++ )
  			//	 {
    				  check_sum =t4ka3cos_r_cur_value+ t4ka3cos_g_cur_value+t4ka3cos_b_cur_value;
			...
			{
				printk("Second OTP\n");
				t4ka3cos_r_cur_value=((read_cmos_sensor(0x4A0C) << 8) | read_cmos_sensor(0x4A0D));
  				t4ka3cos_g_cur_value=((read_cmos_sensor(0x4A0E) << 8) | read_cmos_sensor(0x4A0F));
  				t4ka3cos_b_cur_value=((read_cmos_sensor(0x4A10) << 8) | read_cmos_sensor(0x4A11));
			...
			{
				printk("Third OTP\n");
				t4ka3cos_r_cur_value=((read_cmos_sensor(0x4A14) << 8) | read_cmos_sensor(0x4A15));
  				t4ka3cos_g_cur_value=((read_cmos_sensor(0x4A16) << 8) | read_cmos_sensor(0x4A17));
  				t4ka3cos_b_cur_value=((read_cmos_sensor(0x4A18) << 8) | read_cmos_sensor(0x4A19));
  ...
  //t4ka3cos_g_cur_value=((read_cmos_sensor(0x4A06) << 8) | read_cmos_sensor(0x4A07));
  //t4ka3cos_b_cur_value=((read_cmos_sensor(0x4A08) << 8) | read_cmos_sensor(0x4A09));
  printk("t4ka3cos_r_cur_value=0x%x,t4ka3cos_g_cur_value=0x%x,t4ka3cos_b_cur_value=0x%x\n",t4ka3cos_r_cur_value,t4ka3cos_g_cur_value,t4ka3cos_b_cur_value);
  printk("t4ka3cos_r_golden_value=0x%x,t4ka3cos_g_golden_value=0x%x,t4ka3cos_b_golden_value=0x%x\n",t4ka3cos_r_golden_value,t4ka3cos_g_golden_value,t4ka3cos_b_golden_value);
  //checking awb check sum
  // for( i=0; i<6;  i++ )
  // {
...
//	 printk("otp awb checksum ok!\n");
  ...
  t4ka3cos_r_gain = 256*((t4ka3cos_r_golden_value*t4ka3cos_g_cur_value)*1000/(t4ka3cos_r_cur_value*t4ka3cos_g_golden_value))/1000;
  t4ka3cos_b_gain = 256*((t4ka3cos_b_golden_value*t4ka3cos_g_cur_value)*1000/(t4ka3cos_b_cur_value*t4ka3cos_g_golden_value))/1000;
    printk("((float)t4ka3cos_r_golden_value*t4ka3cos_g_cur_value)=%d,(t4ka3cos_r_cur_value *t4ka3cos_g_golden_value)=%d\n",(t4ka3cos_r_golden_value*t4ka3cos_g_cur_value),(t4ka3cos_r_cur_value *t4ka3cos_g_golden_value));
    printk("(t4ka3cos_b_golden_value*t4ka3cos_g_cur_value)=%d,(t4ka3cos_b_cur_value *t4ka3cos_g_golden_value)=%d\n",(t4ka3cos_b_golden_value*t4ka3cos_g_cur_value),(t4ka3cos_b_cur_value *t4ka3cos_g_golden_value));
	printk("t4ka3cos_r_gain=0x%x,t4ka3cos_b_gain=0x%x\n",t4ka3cos_r_gain,t4ka3cos_b_gain);
	
  //2.write R gain and B gain to KA3 register;
	write_cmos_sensor(0x0210, t4ka3cos_r_gain	>> 8);
	write_cmos_sensor(0x0211, t4ka3cos_r_gain	& 0xff);
	write_cmos_sensor(0x0212, t4ka3cos_b_gain >> 8);
	write_cmos_sensor(0x0213, t4ka3cos_b_gain & 0xff);
	return 0;
 //  }
  //else
  // {
...
static kal_uint16 Get_venderID()
{
	...
	printk("t4ka3cos venderID: 0x%x\n",venderID);
	lensID = read_cmos_sensor(0x4A0A);
	printk("t4ka3cos lesID: 0x%x\n",lensID);
	t4ka3cos_otp_read_enable(0);
	return venderID;
}

static void t4ka3cos_otp_init_setting()
{  
   t4ka3cos_otp_read_enable(1);
   t4ka3cos_otp_checkMag();
   t4ka3cos_otp_update_lsc();
   t4ka3cos_otp_update_awb();
   t4ka3cos_otp_read_enable(0);
}
			...
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			if (*sensor_id == imgsensor_info.sensor_id)
			{
				if(Get_venderID()==0x10)
				{
					if(lensID == 9)
					{
						*sensor_id = T4KA3COSMIPI_SENSOR_ID ;
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	printk("T4KA3COS normal_video  E\n");
...
{
	printk("t4ka3cos  get_resolution E\n");
...
UINT32 T4KA3COS_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
