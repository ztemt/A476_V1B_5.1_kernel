...
#include "s5k4h5ycmipiraw_sub_Sensor_dl.h"

#define PFX "S5K4H5YC_sub_camera_sensor_dl"
...
#define S5K4h5yc_dl_sub_USE_AWB_OTP
#if defined(S5K4h5yc_dl_sub_USE_AWB_OTP)
/*
...
#define RGr_ratio_Typical    383
#define BGr_ratio_Typical    307
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K4H5YC_DL_SUB_SENSOR_ID,
...
*************************************************************************/
#if defined(S5K4h5yc_dl_sub_USE_AWB_OTP)
static kal_uint16 S5K4h5yc_dl_sub_MIPI_read_otp_vendor_id(void)
{
...
#if 0
static void S5K4h5yc_dl_sub_MIPI_read_otp_wb(void)
{
...
static void S5K4h5yc_dl_sub_MIPI_algorithm_otp_wb1()
{
   kal_uint16 R_GAIN = 0, 
   	          B_GAIN = 0,
   	          Gr_GAIN = 0, 
...
static void Darling_SK4H5_read_OTP(struct Darling_SK4H5_otp_struct *SK4H5_OTP)
{
	 ...
	 LOG_INF("xuqian sub [Darling_SK4H5_read_OTP] flag=0x%x, MID=0x%x, LID=0x%x\n", SK4H5_OTP->flag, SK4H5_OTP->MID, SK4H5_OTP->LID);
	 LOG_INF("xuqian sub [Darling_SK4H5_read_OTP] RGr_ratio=0x%x, BGr_ratio=0x%x, GbGr_ratio=0x%x\n", SK4H5_OTP->RGr_ratio, SK4H5_OTP->BGr_ratio, SK4H5_OTP->GbGr_ratio);
	 LOG_INF("xuqian sub [Darling_SK4H5_read_OTP] VCM_start=0x%x, VCM_end=0x%x\n", SK4H5_OTP->VCM_start, SK4H5_OTP->VCM_end);
...
static void Darling_SK4H5_apply_OTP(struct Darling_SK4H5_otp_struct *SK4H5_OTP)
{
   if(((SK4H5_OTP->flag)&0x03)!=0x01) 	return;
	...
	LOG_INF("xuqian sub [Darling_SK4H5_apply_OTP] R_gain=0x%x, B_gain=0x%x, Gb_gain=0x%x, Gr_gain=0x%x\n", R_gain, B_gain, Gb_gain, Gr_gain);
	if(Gr_gain>0x100)
		{
...
#endif
static void S5K4h5yc_dl_sub_MIPI_update_wb_register_from_otp(void)
{   

	LOG_INF("xuqian enter S5K4h5yc_dl_sub_MIPI_update_wb_register_from_otp ========>>>>>>>>\n");
	struct Darling_SK4H5_otp_struct SK4H5_OTP = {0};
   Darling_SK4H5_read_OTP(&SK4H5_OTP);
   Darling_SK4H5_apply_OTP(&SK4H5_OTP);
	LOG_INF("xuqian exit S5K4h5yc_dl_sub_MIPI_update_wb_register_from_otp ========!!!!!!!!\n");
	//return 0;
}
#endif
	...
	//while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            ...
            #if defined(S5K4h5yc_dl_sub_USE_AWB_OTP)
			  if(*sensor_id == 0x485B && S5K4h5yc_dl_sub_MIPI_read_otp_vendor_id() == 0x05)
			  	*sensor_id = 0x485B + 3;
			  else
...
static kal_uint32 open(void)
{
	...
	sensor_init();
    #if defined(S5K4h5yc_dl_sub_USE_AWB_OTP)
    S5K4h5yc_dl_sub_MIPI_update_wb_register_from_otp();
    #endif
	spin_lock(&imgsensor_drv_lock);
...
UINT32 S5K4H5YC_DL_MIPI_RAW_SUB_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
