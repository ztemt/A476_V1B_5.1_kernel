...
#include "t4ka3mipiraw_Sensor_ofilm.h"

extern int iReadReg(u16 a_u2Addr , u8 * a_puBuff , u16 i2cId);
extern int iWriteReg(u16 a_u2Addr , u32 a_u4Data , u32 a_u4Bytes , u16 i2cId);

#define PFX "t4ka3of_4lane_camera_sensor"
...
#define T4KA3OF_OTP_PSEL 0x4A02
#define T4KA3OF_OTP_CTRL 0x4A00

#define T4KA3OF_OTP_DATA_BEGIN_ADDR 0x4A04
#define T4KA3OF_OTP_DATA_END_ADDR   0x4A43
static uint16_t T4KA3OF_OTP_DATA[T4KA3OF_OTP_DATA_END_ADDR - T4KA3OF_OTP_DATA_BEGIN_ADDR + 1] = {0x00};
static uint16_t T4KA3OF_OTP_DATA_BACKUP[T4KA3OF_OTP_DATA_END_ADDR - T4KA3OF_OTP_DATA_BEGIN_ADDR + 1] = {0x00};

static kal_uint16 t4ka3of_r_golden_value=0x16b; //golden module  AWB R value
static kal_uint16 t4ka3of_g_golden_value=0x25d; //golden module  AWB G value: (Gr+Gb)/2
static kal_uint16 t4ka3of_b_golden_value=0x191; //golden module  AWB B value, 
static kal_uint16 t4ka3of_r_cur_value=0; //current module  AWB R value
static kal_uint16 t4ka3of_g_cur_value=0; //current module  AWB G value: (Gr+Gb)/2
static kal_uint16 t4ka3of_b_cur_value=0; //current module  AWB B value
static kal_uint16 t4ka3of_r_gain=0; 
static kal_uint16 t4ka3of_b_gain=0; 
...
static int32_t t4ka3of_otp_read_data(uint16_t* otp_data)
{
    kal_uint16 i = 0;
	
    for (i = 0; i <= (T4KA3OF_OTP_DATA_END_ADDR - T4KA3OF_OTP_DATA_BEGIN_ADDR); i++)
	{
        otp_data[i]=read_cmos_sensor(T4KA3OF_OTP_DATA_BEGIN_ADDR+i);
...
// T4KA3OF Initial 3264x2448_30FPS_MIPI_4LANE(Binning)
...
static void preview_setting(void)
{
   ...
   //t4ka3of preview size 1632x1224 30fps
...
static void t4ka3of_otp_read_enable(kal_uint8 enable)
{
   if(enable)
   	write_cmos_sensor(T4KA3OF_OTP_CTRL,0x01);
   else
   	write_cmos_sensor(T4KA3OF_OTP_CTRL,0x00);
}

static void t4ka3of_otp_set_page(kal_uint16 page)
{
   write_cmos_sensor(T4KA3OF_OTP_PSEL,page);
}

static void t4ka3of_otp_start_read()
{
  write_cmos_sensor(T4KA3OF_OTP_CTRL,(read_cmos_sensor(T4KA3OF_OTP_CTRL) | 0x80));
}

static void t4ka3of_otp_checkMag()
{
  
  kal_uint8 checkMag;
  
  t4ka3of_otp_set_page(0x0f);
  t4ka3of_otp_start_read();
...
static int t4ka3of_otp_update_lsc()
{
	 kal_uint8 lsc_flag;
	 kal_uint8 i;	 
   ...
   {
		t4ka3of_otp_read_enable(1);
		
		t4ka3of_otp_set_page(3);
		t4ka3of_otp_start_read();
		for(i=0;i<64;i++)  
		{
			lsc_buff1[i] = read_cmos_sensor(T4KA3OF_OTP_DATA_BEGIN_ADDR+i);
		}
		
		t4ka3of_otp_set_page(4);
		t4ka3of_otp_start_read();
		for(i=0;i<64;i++)  
		{
			lsc_buff2[i] = read_cmos_sensor(T4KA3OF_OTP_DATA_BEGIN_ADDR+i);
		}
		
		t4ka3of_otp_set_page(5);
		t4ka3of_otp_start_read();
		for(i=0;i<64;i++)  
		{
			lsc_buff3[i] = read_cmos_sensor(T4KA3OF_OTP_DATA_BEGIN_ADDR+i);
		}

		t4ka3of_otp_set_page(6);
		t4ka3of_otp_start_read();
		for(i=0;i<64;i++)  
		{
			lsc_buff4[i] = read_cmos_sensor(T4KA3OF_OTP_DATA_BEGIN_ADDR+i);
		}
...
static void t4ka3of_otp_update_awb()
{  
  int i , j,sum=0,flag;
  kal_uint16 check_sum=0x0000;
  kal_uint8  checksumpage=0x00;
  kal_uint8 OTP_data[64];
  //--------------read OTP Add by joe 2015/9/21--------------//
  t4ka3of_otp_set_page(0x00);
  t4ka3of_otp_start_read();
  flag = read_cmos_sensor(0x4A08);
	mDELAY(2);  
  t4ka3of_otp_set_page(0x02);
  t4ka3of_otp_start_read();
  //t4ka3_otp_read_data(OTP_data); //For printfing all AWB inf
  		if(flag == 0x00)
  			{
				printk("AWB is empty\n");
				return -1; //return for breaking camear off
			}
			
		else if(flag == 0x01)
			{
				printk("first OTP\n");
				t4ka3of_r_cur_value=((read_cmos_sensor(0x4A04) << 8) | read_cmos_sensor(0x4A05));
  				t4ka3of_g_cur_value=((read_cmos_sensor(0x4A06) << 8) | read_cmos_sensor(0x4A07));
  				t4ka3of_b_cur_value=((read_cmos_sensor(0x4A08) << 8) | read_cmos_sensor(0x4A09));
				checksumpage = read_cmos_sensor(0x4A0A);
  				printk("checksumpage=0x%x\n",checksumpage);
				  //checking awb check sum
   			//	for( i=0; i<6;  i++ )
  			//	 {
    				  check_sum =t4ka3of_r_cur_value+ t4ka3of_g_cur_value+t4ka3of_b_cur_value;
			...
			{
				printk("Second OTP\n");
				t4ka3of_r_cur_value=((read_cmos_sensor(0x4A0C) << 8) | read_cmos_sensor(0x4A0D));
  				t4ka3of_g_cur_value=((read_cmos_sensor(0x4A0E) << 8) | read_cmos_sensor(0x4A0F));
  				t4ka3of_b_cur_value=((read_cmos_sensor(0x4A10) << 8) | read_cmos_sensor(0x4A11));
			...
			{
				printk("Third OTP\n");
				t4ka3of_r_cur_value=((read_cmos_sensor(0x4A14) << 8) | read_cmos_sensor(0x4A15));
  				t4ka3of_g_cur_value=((read_cmos_sensor(0x4A16) << 8) | read_cmos_sensor(0x4A17));
  				t4ka3of_b_cur_value=((read_cmos_sensor(0x4A18) << 8) | read_cmos_sensor(0x4A19));
  ...
  printk("t4ka3of_r_cur_value=0x%x,t4ka3of_g_cur_value=0x%x,t4ka3of_b_cur_value=0x%x\n",t4ka3of_r_cur_value,t4ka3of_g_cur_value,t4ka3of_b_cur_value);
  printk("t4ka3of_r_golden_value=0x%x,t4ka3of_g_golden_value=0x%x,t4ka3of_b_golden_value=0x%x\n",t4ka3of_r_golden_value,t4ka3of_g_golden_value,t4ka3of_b_golden_value);
  //checking awb check sum
  // for( i=0; i<6;  i++ )
  // {
...
//	 printk("otp awb checksum ok!\n");
  ...
  t4ka3of_r_gain = 256*((t4ka3of_r_golden_value*t4ka3of_g_cur_value)*1000/(t4ka3of_r_cur_value*t4ka3of_g_golden_value))/1000;
  t4ka3of_b_gain = 256*((t4ka3of_b_golden_value*t4ka3of_g_cur_value)*1000/(t4ka3of_b_cur_value*t4ka3of_g_golden_value))/1000;
    printk("((float)t4ka3of_r_golden_value*t4ka3of_g_cur_value)=%d,(t4ka3of_r_cur_value *t4ka3of_g_golden_value)=%d\n",(t4ka3of_r_golden_value*t4ka3of_g_cur_value),(t4ka3of_r_cur_value *t4ka3of_g_golden_value));
    printk("(t4ka3of_b_golden_value*t4ka3of_g_cur_value)=%d,(t4ka3of_b_cur_value *t4ka3of_g_golden_value)=%d\n",(t4ka3of_b_golden_value*t4ka3of_g_cur_value),(t4ka3of_b_cur_value *t4ka3of_g_golden_value));
	printk("t4ka3of_r_gain=0x%x,t4ka3of_b_gain=0x%x\n",t4ka3of_r_gain,t4ka3of_b_gain);
	
  //2.write R gain and B gain to KA3 register;
	write_cmos_sensor(0x0210, t4ka3of_r_gain	>> 8);
	write_cmos_sensor(0x0211, t4ka3of_r_gain	& 0xff);
	write_cmos_sensor(0x0212, t4ka3of_b_gain >> 8);
	write_cmos_sensor(0x0213, t4ka3of_b_gain & 0xff);
	return 0;
 //  }
  //else
  // {
...
static void t4ka3of_otp_init_setting()
{  
   t4ka3of_otp_read_enable(1);
   t4ka3of_otp_checkMag();
   t4ka3of_otp_update_lsc();
   t4ka3of_otp_update_awb();
   t4ka3of_otp_read_enable(0);
}
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			if (*sensor_id == imgsensor_info.sensor_id)
				{
				if(af_ofilm_af_test() == 0)
				{
					*sensor_id=T4KA3OFMIPI_SENSOR_ID;
...
UINT32 T4KA3OF_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
