...
#include "t4ka3mipirawsub_Sensor.h"

extern int iReadReg(u16 a_u2Addr , u8 * a_puBuff , u16 i2cId);
extern int iWriteReg(u16 a_u2Addr , u32 a_u4Data , u32 a_u4Bytes , u16 i2cId);

#define PFX "t4ka3_2lane_camera_sensor"
...
#define T4KA3SUB_OTP_PSEL 0x4A02
#define T4KA3SUB_OTP_CTRL 0x4A00

#define T4KA3SUB_OTP_DATA_BEGIN_ADDR 0x4A04
#define T4KA3SUB_OTP_DATA_END_ADDR   0x4A43
static uint16_t T4KA3SUB_OTP_DATA[T4KA3SUB_OTP_DATA_END_ADDR - T4KA3SUB_OTP_DATA_BEGIN_ADDR + 1] = {0x00};
static uint16_t T4KA3SUB_OTP_DATA_BACKUP[T4KA3SUB_OTP_DATA_END_ADDR - T4KA3SUB_OTP_DATA_BEGIN_ADDR + 1] = {0x00};
...
static kal_uint16 t4ka3_r_golden_value=0x5e; //golden module  AWB R value
static kal_uint16 t4ka3_g_golden_value=0x93; //golden module  AWB G value: (Gr+Gb)/2
...
static int32_t t4ka3_otp_read_data(uint16_t* otp_data)
{
    ...
    for (i = 0; i <= (T4KA3SUB_OTP_DATA_END_ADDR - T4KA3SUB_OTP_DATA_BEGIN_ADDR); i++)
	{
        otp_data[i]=read_cmos_sensor(T4KA3SUB_OTP_DATA_BEGIN_ADDR+i);
...
// T4KA3SUB Initial 3264x2448_30FPS_MIPI_2LANE(Binning)
...
// made from Register setter ver600
  ...
  write_cmos_sensor(0x3393,0x01);//Reserved;
	...
	write_cmos_sensor(0x3390,0x16);//only for MIPI two lane
  write_cmos_sensor(0x3391,0x1c);
	write_cmos_sensor(0x3392,0x1c);
	...
	write_cmos_sensor(0x0114,0x01);//CSI_LANE_MODE[1:0];
	write_cmos_sensor(0x4136,0x18);//EXTCLK_FRWQ_MHZ[15:8];
	write_cmos_sensor(0x4137,0x00);//EXTCLK_FRWQ_MHZ[7:0];
	write_cmos_sensor(0x0820,0x09);//MSB_LBRATE[31:24];
	write_cmos_sensor(0x0821,0x60);//MSB_LBRATE[23:16];
	...
	write_cmos_sensor(0x0306,0x03);//-/-/-/-/-/PLL_MULTIPLIER[10:8];
	write_cmos_sensor(0x0307,0x22);//PLL_MULTIPLIER[7:0];
...
{
   if(enable)
   	write_cmos_sensor(T4KA3SUB_OTP_CTRL,0x01);
   else
   	write_cmos_sensor(T4KA3SUB_OTP_CTRL,0x00);
}

static void t4ka3_otp_set_page(kal_uint16 page)
{
   write_cmos_sensor(T4KA3SUB_OTP_PSEL,page);
}

static void t4ka3_otp_start_read()
{
  write_cmos_sensor(T4KA3SUB_OTP_CTRL,(read_cmos_sensor(T4KA3SUB_OTP_CTRL) | 0x80));
}

static void t4ka3_otp_checkMag()
{
...
static int t4ka3_otp_update_lsc()
{
	 kal_uint8 lsc_flag;
	 kal_uint8 i;	 
   ...
   else if(lsc_flag==0x07)
   {
		...
		for(i=0;i<64;i++)  
		{
			lsc_buff1[i] = read_cmos_sensor(T4KA3SUB_OTP_DATA_BEGIN_ADDR+i);
		}
		...
		for(i=0;i<64;i++)  
		{
			lsc_buff2[i] = read_cmos_sensor(T4KA3SUB_OTP_DATA_BEGIN_ADDR+i);
		}
		...
		for(i=0;i<64;i++)  
		{
			lsc_buff3[i] = read_cmos_sensor(T4KA3SUB_OTP_DATA_BEGIN_ADDR+i);
		}
		...
		for(i=0;i<64;i++)  
		{
			lsc_buff4[i] = read_cmos_sensor(T4KA3SUB_OTP_DATA_BEGIN_ADDR+i);
		}
...
static void t4ka3_otp_update_awb()
{  
  int i , j,sum=0,flag;
...
static void t4ka3_otp_init_setting()
{  
   t4ka3_otp_read_enable(1);
   t4ka3_otp_checkMag();
			...
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			if (*sensor_id == imgsensor_info.sensor_id)
				...
				if(af_ofilm_af_test()!=0)
				{
					if(af_sjc_af_test()!=0)	
					{
					   *sensor_id = T4KA3SUBMIPI_SENSOR_ID;
...
static kal_uint32 open(void)
{
	...
	printk("PLATFORM:MT6735M,MIPI 2LANE\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			af_sjc_af_test();
			if (sensor_id == imgsensor_info.sensor_id) {				
				printk("open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);	  
				break;
			}	
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	printk("T4KA3SUB normal_video  E\n");
...
UINT32 T4KA3SUB_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
