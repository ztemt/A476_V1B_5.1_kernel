...
#define S5K4h5yc_USE_AWB_OTP
#if defined(S5K4h5yc_USE_AWB_OTP)
/*
...
*************************************************************************/
#if defined(S5K4h5yc_USE_AWB_OTP)
static kal_uint16 S5K4h5yc_MIPI_read_otp_vendor_id(void)
{
...
static void SK4H5_read_OTP(struct SK4H5_otp_struct *SK4H5_OTP)
{
	 ...
	 LOG_INF("xuqian [sunwin_SK4H5_read_OTP] flag=0x%x, MID=0x%x, LID=0x%x\n", SK4H5_OTP->flag, SK4H5_OTP->MID, SK4H5_OTP->LID);
	 LOG_INF("xuqian [sunwin_SK4H5_read_OTP] RGr_ratio=0x%x, BGr_ratio=0x%x, GbGr_ratio=0x%x\n", SK4H5_OTP->RGr_ratio, SK4H5_OTP->BGr_ratio, SK4H5_OTP->GbGr_ratio);
	 //LOG_INF("xuqian [sunwin_SK4H5_read_OTP] VCM_start=0x%x, VCM_end=0x%x\n", SK4H5_OTP->VCM_start, SK4H5_OTP->VCM_end);
...
static void SK4H5_apply_OTP(struct SK4H5_otp_struct *SK4H5_OTP)
{
   if(((SK4H5_OTP->flag)&0x03)!=0x01) 	return;
	...
	LOG_INF("xuqian [sunwin_SK4H5_apply_OTP] R_gain=0x%x, B_gain=0x%x, Gb_gain=0x%x, Gr_gain=0x%x\n", R_gain, B_gain, Gb_gain, Gr_gain);
	if(Gr_gain>0x100)
		{
...
static void S5K4h5yc_MIPI_update_wb_register_from_otp(void)
{   
	struct SK4H5_otp_struct SK4H5_OTP_current = {0};
	LOG_INF("xuqian enter S5K4h5yc_MIPI_update_wb_register_from_otp ========>>>>>>>>\n"); 
	...
	//while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            ...
            #if defined(S5K4h5yc_USE_AWB_OTP)
			  if(*sensor_id == 0x485B && S5K4h5yc_MIPI_read_otp_vendor_id() == 0x04)
			  	*sensor_id = 0x485B;
			  else
...
static kal_uint32 open(void)
{
	...
	sensor_init();
    #if defined(S5K4h5yc_USE_AWB_OTP)
    S5K4h5yc_MIPI_update_wb_register_from_otp();
    #endif
	spin_lock(&imgsensor_drv_lock);
