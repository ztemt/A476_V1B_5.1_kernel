...
#include "s5k4h5ycmipiraw_sub_Sensor.h"

#define PFX "S5K4H5YC_sub_camera_sensor"
...
#define S5K4h5yc_sub_USE_AWB_OTP
#if defined(S5K4h5yc_sub_USE_AWB_OTP)
/*
#define GAIN_DEFAULT       	     0x0100
static kal_uint16 r_ratio = 0;
static kal_uint16 b_ratio = 0;
*/
static struct SK4H5_otp_struct {
int flag; 
int MID;
int LID;
int RGr_ratio;
...
#define RGr_ratio_Typical    865 // golden
#define BGr_ratio_Typical    680 // golden
#define GbGr_ratio_Typical   1024 // golden
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K4H5YC_SUB_SENSOR_ID,
...
*************************************************************************/
#if defined(S5K4h5yc_sub_USE_AWB_OTP)
static kal_uint16 S5K4h5yc_sub_MIPI_read_otp_vendor_id(void)
{
	...
	switch (info_flag&0xf0)
    {  
      case 0x40:
       ModuleHouseID =  read_cmos_sensor(0x3a08);
       break;
      case 0x10:
       ModuleHouseID =  read_cmos_sensor(0x3a1D);
       break;
      default:	 
...
static void SK4H5_read_OTP(struct SK4H5_otp_struct *SK4H5_OTP)
{
	 ...
	 if((val&0xf0)==0x40)
	 	{
	 	SK4H5_OTP->flag = 0x01;
		SK4H5_OTP->MID = SK4H5_read_cmos_sensor(0x3A08);
		SK4H5_OTP->LID = SK4H5_read_cmos_sensor(0x3A09);
		SK4H5_OTP->RGr_ratio = (SK4H5_read_cmos_sensor(0x3A0d)<<8)+SK4H5_read_cmos_sensor(0x3A0e);
		SK4H5_OTP->BGr_ratio = (SK4H5_read_cmos_sensor(0x3A0f)<<8)+SK4H5_read_cmos_sensor(0x3A10);
		SK4H5_OTP->GbGr_ratio = (SK4H5_read_cmos_sensor(0x3A11)<<8)+SK4H5_read_cmos_sensor(0x3A12);
	 	}
	 else if((val&0xf0)==0x01)
	 	{
	 	SK4H5_OTP->flag=0x01;
		SK4H5_OTP->MID = SK4H5_read_cmos_sensor(0x3A1D);
		SK4H5_OTP->LID = SK4H5_read_cmos_sensor(0x3A1E);
		SK4H5_OTP->RGr_ratio = (SK4H5_read_cmos_sensor(0x3A22)<<8)+SK4H5_read_cmos_sensor(0x3A23);
		SK4H5_OTP->BGr_ratio = (SK4H5_read_cmos_sensor(0x3A24)<<8)+SK4H5_read_cmos_sensor(0x3A25);
		SK4H5_OTP->GbGr_ratio = (SK4H5_read_cmos_sensor(0x3A26)<<8)+SK4H5_read_cmos_sensor(0x3A27);
	 	}
	 else
	 	{
	 	SK4H5_OTP->flag=0x00;
	 ...
	 LOG_INF("xuqian [sunwin_sub_SK4H5_read_OTP] flag=0x%x, MID=0x%x, LID=0x%x\n", SK4H5_OTP->flag, SK4H5_OTP->MID, SK4H5_OTP->LID);
	 LOG_INF("xuqian [sunwin_sub_SK4H5_read_OTP] RGr_ratio=0x%x, BGr_ratio=0x%x, GbGr_ratio=0x%x\n", SK4H5_OTP->RGr_ratio, SK4H5_OTP->BGr_ratio, SK4H5_OTP->GbGr_ratio);
	 //LOG_INF("xuqian [sunwin_sub_SK4H5_read_OTP] VCM_start=0x%x, VCM_end=0x%x\n", SK4H5_OTP->VCM_start, SK4H5_OTP->VCM_end);
	 SK4H5_write_cmos_sensor(0x3A00,0x00);
}    

static void SK4H5_apply_OTP(struct SK4H5_otp_struct *SK4H5_OTP)
{
   if(((SK4H5_OTP->flag)&0x03)!=0x01) 	return;
   
 	int R_gain,B_gain,Gb_gain,Gr_gain,Base_gain;
	R_gain = (RGr_ratio_Typical*0x100)/SK4H5_OTP->RGr_ratio;
	B_gain = (BGr_ratio_Typical*0x100)/SK4H5_OTP->BGr_ratio;
	Gb_gain = (GbGr_ratio_Typical*0x100)/SK4H5_OTP->GbGr_ratio;
	Gr_gain = 0x100;
       Base_gain = R_gain;
	...
	LOG_INF("xuqian [sunwin_sub_SK4H5_apply_OTP] R_gain=0x%x, B_gain=0x%x, Gb_gain=0x%x, Gr_gain=0x%x\n", R_gain, B_gain, Gb_gain, Gr_gain);
	if(Gr_gain>0x100)
		{
...
static void S5K4h5yc_sub_MIPI_update_wb_register_from_otp(void)
{  
	struct SK4H5_otp_struct SK4H5_OTP_current = {0};
	LOG_INF("xuqian enter S5K4h5yc_sub_MIPI_update_wb_register_from_otp ========>>>>>>>>\n"); 
   SK4H5_read_OTP(&SK4H5_OTP_current);
   SK4H5_apply_OTP(&SK4H5_OTP_current);
   LOG_INF("xuqian enter S5K4h5yc_sub_MIPI_update_wb_register_from_otp ========!!!!!!!!\n");
}
#endif
	...
	//while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            ...
            #if defined(S5K4h5yc_sub_USE_AWB_OTP)
			  if(*sensor_id == 0x485B && S5K4h5yc_sub_MIPI_read_otp_vendor_id() == 0x03)
			  	*sensor_id = 0x485B + 1;
			  else
...
static kal_uint32 open(void)
{
	...
	sensor_init();
    #if defined(S5K4h5yc_sub_USE_AWB_OTP)
    S5K4h5yc_sub_MIPI_update_wb_register_from_otp();
    #endif
	spin_lock(&imgsensor_drv_lock);
...
UINT32 S5K4H5YC_MIPI_RAW_SUB_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
