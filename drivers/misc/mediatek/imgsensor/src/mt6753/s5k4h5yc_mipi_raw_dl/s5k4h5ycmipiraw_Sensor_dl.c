...
#include "s5k4h5ycmipiraw_Sensor_dl.h"

#define PFX "S5K4H5YC_camera_sensor_dl"
...
#define S5K4h5yc_dl_USE_AWB_OTP
#if defined(S5K4h5yc_dl_USE_AWB_OTP)
/*
#define GAIN_DEFAULT       	     0x0100
static kal_uint16 r_ratio = 0x183;//387   //  0x183
static kal_uint16 b_ratio = 0x135;//309  //  0x135
*/
static struct Darling_SK4H5_otp_struct {
int flag; 
int MID;
int LID;
int RGr_ratio;
...
#define RGr_ratio_Typical    387
#define BGr_ratio_Typical    309
#define GbGr_ratio_Typical  512

#define SK4H5_write_cmos_sensor(addr, para)  write_cmos_sensor((u16) addr , (u32) para)
#define SK4H5_read_cmos_sensor(addr) read_cmos_sensor((u32) addr)
#endif


static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K4H5YC_DL_SENSOR_ID,
...
{ 
	...
	}   /*  S5K4H5YCMIPI_dl_Capture_Setting  */
...
static void capture_setting(kal_uint16 currefps)
{
	...
	// Image Orientation -------------------------------
	write_cmos_sensor(0x0101,0x03); 	//	[1:0]	image_orientation ([0] mirror en, [1] flip en)
...
{              LOG_INF("E//VGA 120fps");
			...
			write_cmos_sensor(0x0101,0x03);       //Image  orientation	image_orientation ([0] mirror en, [1] flip en)
			...
			write_cmos_sensor(0x0340,0x04);  //02     //Frame Rate 	frame_length_lines H                                
			write_cmos_sensor(0x0341,0xF0);  //80   	//frame_length_lines L                                            
			...
			write_cmos_sensor(0x034C,0x06);  //03   	//x_output_size H                                                 
			write_cmos_sensor(0x034D,0x68);  //34   	//x_output_size L                                                 
			write_cmos_sensor(0x034E,0x04);  //02   	//y_output_size H                                                 
			write_cmos_sensor(0x034F,0xD0);  //68   	//y_output_size L                                                 
...
*************************************************************************/
#if defined(S5K4h5yc_dl_USE_AWB_OTP)
static kal_uint16 S5K4h5yc_dl_MIPI_read_otp_vendor_id(void)
{
	uint16_t ModuleHouseID = 0, 
			 info_flag = 0;
     write_cmos_sensor(0x3a02, 0x00); //page set  
     write_cmos_sensor(0x3a00, 0x01); //otp enable read
	 info_flag = read_cmos_sensor(0x3A04);
	printk("%s,info_flag = 0x%x\n",__func__,info_flag);
	switch (info_flag)
    {  
      case 0x40:
       ModuleHouseID =  read_cmos_sensor(0x3a05);
       break;
      case 0x10:
       ModuleHouseID =  read_cmos_sensor(0x3a09);
       break;
      default:	 
      LOG_INF("liukun S5K4H5_Sensor: info_flag error value: %d \n ",info_flag);
      break;
    }
...
#if 0
static void S5K4h5yc_dl_MIPI_read_otp_wb(void)
{
    ...
    uint16_t wb_flag = 0; 
        	   write_cmos_sensor(0x3a02, 0x00); //page set  
        	   write_cmos_sensor(0x3a00, 0x01); //otp enable read
			   wb_flag = read_cmos_sensor(0x3A0D);
			   LOG_INF("liukun %s,wb_flag = 0x%x\n",__func__,wb_flag);
			   switch (wb_flag)
			  {  
				case 0x40:
        		current_R =  read_cmos_sensor(0x3a0e);
        		current_Gr = read_cmos_sensor(0x3a0f);
        		current_Gb = read_cmos_sensor(0x3a10);
        		current_B =  read_cmos_sensor(0x3a11); 
				golden_R =  read_cmos_sensor(0x3a12);
			    golden_Gr = read_cmos_sensor(0x3a13);
			    golden_Gb = read_cmos_sensor(0x3a14);
				golden_B =  read_cmos_sensor(0x3a15); 
				break;
				case 0x10:
        		current_R =  read_cmos_sensor(0x3a16);
        		current_Gr = read_cmos_sensor(0x3a17);
        		current_Gb = read_cmos_sensor(0x3a18);
        		current_B =  read_cmos_sensor(0x3a19); 
				golden_R =  read_cmos_sensor(0x3a1a);
			    golden_Gr = read_cmos_sensor(0x3a1b);
			    golden_Gb = read_cmos_sensor(0x3a1c);
				golden_B =  read_cmos_sensor(0x3a1d); 
				break;
				default:	 
				LOG_INF("liukun S5K4H5_Sensor: wb_flag error value: %d \n ",wb_flag);
				break;
			  }
			  write_cmos_sensor(0x3a00, 0x00); //otp disable read
  		LOG_INF("liukun [S5K4h5yc_dl_MIPI_read_otp_wb] current_R=0x%x, current_Gr=0x%x, current_Gb=0x%x, current_B=0x%x\n", current_R, current_Gr, current_Gb, current_B);
   		...
   		LOG_INF("liukun [S5K4h5yc_dl_MIPI_read_otp_wb] r_ratio=0x%x, b_ratio=0x%x\n", r_ratio, b_ratio);      
}
static void S5K4h5yc_dl_MIPI_algorithm_otp_wb1()
{
   kal_uint16 R_GAIN = 0, 
   	          B_GAIN = 0,
   	          Gr_GAIN = 0, 
   ...
   LOG_INF("[S5K4h5yc_dl] [S5K4h5yc_dl_MIPI_algorithm_otp_wb1] R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", R_GAIN, B_GAIN, G_GAIN);    
}
#else
static void Darling_SK4H5_read_OTP(struct Darling_SK4H5_otp_struct *SK4H5_OTP)
{
	 ...
	 LOG_INF("xuqian [Darling_SK4H5_read_OTP] flag=0x%x, MID=0x%x, LID=0x%x\n", SK4H5_OTP->flag, SK4H5_OTP->MID, SK4H5_OTP->LID);
	 LOG_INF("xuqian [Darling_SK4H5_read_OTP] RGr_ratio=0x%x, BGr_ratio=0x%x, GbGr_ratio=0x%x\n", SK4H5_OTP->RGr_ratio, SK4H5_OTP->BGr_ratio, SK4H5_OTP->GbGr_ratio);
	 LOG_INF("xuqian [Darling_SK4H5_read_OTP] VCM_start=0x%x, VCM_end=0x%x\n", SK4H5_OTP->VCM_start, SK4H5_OTP->VCM_end);
	 SK4H5_write_cmos_sensor(0x3A00,0x00);
}

static void Darling_SK4H5_apply_OTP(struct Darling_SK4H5_otp_struct *SK4H5_OTP)
{
   if(((SK4H5_OTP->flag)&0x03)!=0x01) 	return;
	...
	LOG_INF("xuqian [Darling_SK4H5_apply_OTP] R_gain=0x%x, B_gain=0x%x, Gb_gain=0x%x, Gr_gain=0x%x\n", R_gain, B_gain, Gb_gain, Gr_gain);
	if(Gr_gain>0x100)
		{
...
#endif
static void S5K4h5yc_dl_MIPI_update_wb_register_from_otp(void)
{   

	LOG_INF("xuqian enter S5K4h5yc_dl_MIPI_update_wb_register_from_otp ========>>>>>>>>\n");
	struct Darling_SK4H5_otp_struct SK4H5_OTP = {0};
   Darling_SK4H5_read_OTP(&SK4H5_OTP);
   Darling_SK4H5_apply_OTP(&SK4H5_OTP);
	LOG_INF("xuqian exit S5K4h5yc_dl_MIPI_update_wb_register_from_otp ========!!!!!!!!\n");
	//return 0;
}
#endif
	...
	//while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            ...
            #if defined(S5K4h5yc_dl_USE_AWB_OTP)
			  if(*sensor_id == 0x485B && S5K4h5yc_dl_MIPI_read_otp_vendor_id() == 0x05)
			  	*sensor_id = 0x485B + 2;
			  else
...
static kal_uint32 open(void)
{
	...
	sensor_init();
    #if defined(S5K4h5yc_dl_USE_AWB_OTP)
    S5K4h5yc_dl_MIPI_update_wb_register_from_otp();
    #endif
	spin_lock(&imgsensor_drv_lock);
...
UINT32 S5K4H5YC_DL_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
