...
static struct i2c_board_info __initdata i2c_kxcjk={ I2C_BOARD_INFO("KXCJK", (0x1E>>1))};
...
static struct i2c_driver kxcjk_i2c_driver = {
 ...
 struct acc_init_info kxcjk_init_info = {
		.name = "kxcjk",
...
static int kxcjk_gsensor_enable_nodata(int en)
{
    int err = 0;
	...
	else
	{
		...
		{
			err = KXCJK_SetPowerMode(obj_i2c_data->client, enable_status);
...
static int kxcjk_gsensor_set_delay(u64 ns)
{
		int err = 0;
		int value;
		...
		err = KXCJK_SetBWRate(obj_i2c_data->client, sample_delay);
		...
		{	
			...
			obj_i2c_data->fir.sum[KXCJK_AXIS_X] = 0;
			obj_i2c_data->fir.sum[KXCJK_AXIS_Y] = 0;
			obj_i2c_data->fir.sum[KXCJK_AXIS_Z] = 0;
...
static int kxcjk_gsensor_get_data(int* x ,int* y,int* z, int* status)
{
    ...
    mutex_lock(&gsensor_mutex);
	KXCJK_ReadSensorData(obj_i2c_data->client, buff, KXCJK_BUFSIZE);
	mutex_unlock(&gsensor_mutex);
...
static int kxcjk_gsensor_open_report_data(int open)
{
...
*/
	ctl.open_report_data= kxcjk_gsensor_open_report_data;
	ctl.enable_nodata = kxcjk_gsensor_enable_nodata;
	ctl.set_delay  = kxcjk_gsensor_set_delay;
	//ctl.batch = gsensor_set_batch;
	ctl.is_report_input_direct = false;

	err = acc_register_control_path(&ctl);
	
    data.get_data = kxcjk_gsensor_get_data;
	...
	printk("\r\n zhaoshaopeng kxcjk_i2c_probe 22 \r\n");
...
static int  kxcjk_local_init(void)
{
   struct acc_hw *hw = kxcjk_get_cust_acc_hw();
	GSE_FUN();
	printk("### leanda kxcjk_local_init \r\n");
...
static int __init kxcjk_init(void)
{
	...
	acc_driver_add(&kxcjk_init_info);
