...
static DEFINE_MUTEX(kx023_mutex);
...
/*----------------------------------------------------------------------------*/
static struct acc_init_info kx023_init_info = {
		.name = "kx023",
...
static int kx023_gsensor_open_report_data(int open)
{
	//should queuq work to report event if  is_report_input_direct=true
	return 0;
}

static int kx023_gsensor_enable_nodata(int value)
{
//	value = *(int *)buff_in;
	struct kx023_i2c_data *priv = obj_i2c_data;

	mutex_lock(&kx023_mutex);
	...
	{
		KX023_SetPowerMode( priv->client, !sensor_power);
	}
	mutex_unlock(&kx023_mutex);	 
	return 0;
}

static int kx023_gsensor_set_delay(u64 ns)
{
	int err;
	struct kx023_i2c_data *priv = obj_i2c_data;
...
static int kx023_gsensor_get_data(int* x ,int* y,int* z, int* status)
{
...
//	}
	...
	ctl.open_report_data= kx023_gsensor_open_report_data;
	ctl.enable_nodata = kx023_gsensor_enable_nodata;
	ctl.set_delay  = kx023_gsensor_set_delay;
    ...
    data.get_data = kx023_gsensor_get_data;
