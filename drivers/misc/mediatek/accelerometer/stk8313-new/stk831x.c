...
#endif
extern 	struct acc_hw* stk831x_get_cust_acc_hw(void);
/*----------------------------------------------------------------------------*/
#define DEBUG 1
/*----------------------------------------------------------------------------*/
...
static int 	stk831x_init_flag = -1;// 0<==>OK -1 <==> fail

static struct acc_init_info stk831x_init_info = {
...
#define GSE_FUN(f)               GSE_LOG(GSE_TAG"%s\n", __FUNCTION__)
...
static struct data_resolution stk831x_data_resolution[] = {
	...
	{{ 46, 88}, 21.33},   /*+/-1.5g  in 6-bit resolution:  46.88 mg/LSB*/    
	{{ 46, 88}, 21.33},   /*+/-6g  in 8-bit resolution:  46.88 mg/LSB*/
	{{125, 0},  8},   /*+/-16g  in 8-bit resolution: 125 mg/LSB*/
};
#elif defined SENSOR_STK8313
static struct data_resolution stk831x_data_resolution[] = {
...
/*----------------------------------------------------------------------------*/
#ifdef SENSOR_STK8312
static struct data_resolution stk831x_offset_resolution = {{46, 88}, 21.33};
#elif defined SENSOR_STK8313
...
static int stk_store_in_ic( struct i2c_client *client, char otp_offset[], char FT_index, uint32_t delay_ms)
{
	...
	//GSE_LOG("%s:Check All OTP Data after write 0x2B 0x2F 0x33\n", __func__);
	//STK831x_ReadAllOTP();
#endif	
	...
	do
	{
		...
		do 
		{
			...
			if(buffer[0]& 0x80)
			{
				...
				{
#ifdef STK_DEBUG_CALI					
					GSE_LOG("%s: write 0x%x=0x%x successfully\n", __func__, wReg, value);
#endif			
					re_write = 0xFF;
					break;
				}
...
static int STK831x_SetCali(struct i2c_client *client, char sstate)
{
	...
	for(state=0;state<3;state++)
	{
		...
		acc_ave[2] += acc_adc[2];
#ifdef STK_DEBUG_CALI				
		GSE_LOG("%s:before cali, acc=%d,%d,%d\n", __func__, acc_adc[0], acc_adc[1], acc_adc[2]);	
#endif		
	...
	if(acc_ave[2] <= -1)
		...
		GSE_LOG("%s:acc_ave[2]=0, use default stk831x_placement\n", __func__);
#ifdef STK_DEBUG_CALI			
	GSE_LOG("%s:stk831x_placement=%d\n", __func__, stk831x_placement);
#endif	
	acc_ave[2] = 0;
	...
	if(store_location == 1)
	{
		...
		else
		{
			...
			if(result)
			{
				GSE_LOG("%s:write calibration failed\n", __func__);
				atomic_set(&cali_status, -result);				
			}
			else
			{
...
static int STK831x_GetCali(struct i2c_client *client)
{
...
#ifdef STK_TUNE		
	GSE_LOG("%s: stk_tune_done=%d, stk_tune_index=%d, stk_tune_offset=%d,%d,%d\n", __func__, 
	...
	if ((stk_get_file_content(r_buf, STK_ACC_CALI_FILE_SIZE)) == 0)
	{
		if(r_buf[0] == STK_ACC_CALI_VER0 && r_buf[1] == STK_ACC_CALI_VER1)
		{
			...
			offset[2] = r_buf[4];
			mode = r_buf[5];
			GSE_LOG("%s:file offset:%#02x,%#02x,%#02x,%#02x\n", 
				__func__, offset[0], offset[1], offset[2], mode);							
		}
		else
		{
	...
	GSE_LOG("%s: OTP 0x43-0x49:%#02x,%#02x,%#02x,%#02x,%#02x,%#02x\n", __func__, regR[0], 
	...
	GSE_LOG("%s: OTP 0x53-0x59:%#02x,%#02x,%#02x,%#02x,%#02x,%#02x\n", __func__, regR[0], 
		regR[1], regR[2],regR[3], regR[4], regR[5]);
...
static int STK831x_GetCode(int a0, int a1)
{
...
static void STK831x_CheckInit(void)
{
	int i,j;
	
	//memset((void *)stkcheckcode, 0x00, sizeof(char)*22*22); 	
	if(stkcheckcode[0][0]==0)
	{
		for(i=0;i<CHECK_CODE_SIZE;i++)
			for(j=0;j<CHECK_CODE_SIZE;j++)
				stkcheckcode[i][j] = (char)STK831x_GetCode(i,j);
	}	
}
static int STK831x_CheckCode(int acc[])
{
	...
	if(a>=CHECK_CODE_SIZE || b>=CHECK_CODE_SIZE)
		acc[2] = 0;
	else if(obj->hw->direction<4)
		acc[2] = ((int)stkcheckcode[a][b]);
	else
		acc[2] = -((int)stkcheckcode[a][b]);
	return 0;	
}
	...
	if(clear)
	{
		if(check_result == 3)
		{
			...
			if(acc[0] != verifing[0] && acc[0] != verifing[1] && acc[1] != verifing[0]
					&& acc[1] != verifing[1])
			{
				atomic_set(&obj->event_since_en_limit, 21); 
				if(atomic_read(&obj->firlen) < 2)
				{
					atomic_set(&obj->firlen, 2);
...
//		(buf[STK831X_AXIS_Z*2+1]));
		...
		else if(atomic_read(&obj->event_since_en_limit) == 21)
			STK831x_CheckCode(acc_xyz);			
		...
		if(atomic_read(&obj->filter))
		{
			if(atomic_read(&obj->fir_en) && !atomic_read(&obj->suspend))
			{
				...
				{                
					obj->fir.raw[obj->fir.num][STK831X_AXIS_X] = (s16) acc_xyz[STK831X_AXIS_X];
					obj->fir.raw[obj->fir.num][STK831X_AXIS_Y] = (s16) acc_xyz[STK831X_AXIS_Y];
					obj->fir.raw[obj->fir.num][STK831X_AXIS_Z] = (s16) acc_xyz[STK831X_AXIS_Z];
					obj->fir.sum[STK831X_AXIS_X] += acc_xyz[STK831X_AXIS_X];
					obj->fir.sum[STK831X_AXIS_Y] += acc_xyz[STK831X_AXIS_Y];
					obj->fir.sum[STK831X_AXIS_Z] += acc_xyz[STK831X_AXIS_Z];
				...
				else
				{
					...
					obj->fir.raw[idx][STK831X_AXIS_X] = (s16) acc_xyz[STK831X_AXIS_X];
					obj->fir.raw[idx][STK831X_AXIS_Y] = (s16) acc_xyz[STK831X_AXIS_Y];
					obj->fir.raw[idx][STK831X_AXIS_Z] = (s16) acc_xyz[STK831X_AXIS_Z];
					obj->fir.sum[STK831X_AXIS_X] += acc_xyz[STK831X_AXIS_X];
					obj->fir.sum[STK831X_AXIS_Y] += acc_xyz[STK831X_AXIS_Y];
					obj->fir.sum[STK831X_AXIS_Z] += acc_xyz[STK831X_AXIS_Z];
					obj->fir.idx++;

					acc_xyz[STK831X_AXIS_X] = obj->fir.sum[STK831X_AXIS_X]/firlen;
					acc_xyz[STK831X_AXIS_Y] = obj->fir.sum[STK831X_AXIS_Y]/firlen;
					acc_xyz[STK831X_AXIS_Z] = obj->fir.sum[STK831X_AXIS_Z]/firlen;
					...
					if(atomic_read(&obj->trace) & ADX_TRC_FILTER)
					{
						...
						obj->fir.sum[STK831X_AXIS_X], obj->fir.sum[STK831X_AXIS_Y], obj->fir.sum[STK831X_AXIS_Z],
						acc_xyz[STK831X_AXIS_X], acc_xyz[STK831X_AXIS_Y], acc_xyz[STK831X_AXIS_Z]);
					}
				}
			}
		}	
...
#ifdef STK_ZG_FILTER	
		if( abs(acc_xyz[STK831X_AXIS_X]) <= STK_ZG_COUNT)	
			acc_xyz[STK831X_AXIS_X] = (acc_xyz[STK831X_AXIS_X]*zero_fir);	
		if( abs(acc_xyz[STK831X_AXIS_Y]) <= STK_ZG_COUNT)
			acc_xyz[STK831X_AXIS_Y] = (acc_xyz[STK831X_AXIS_Y]*zero_fir);
		if( abs(acc_xyz[STK831X_AXIS_Z]) <= STK_ZG_COUNT)
			acc_xyz[STK831X_AXIS_Z] = (acc_xyz[STK831X_AXIS_Z]*zero_fir);	
#endif			
...
static int STK831X_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	/*
	if((sensor_power == false) && (atomic_read(&obj->suspend)==0) )
	{
...
static ssize_t stk831x_show_allreg(struct device_driver *ddri, char *buf)
{
	...
	{
		buffer[0] = bb * 0x10;
		//GSE_LOG("%s: read (%d) buffer[0] = 0x%x\n", __func__, buffer, buffer[0]);
		...
		buffer[8] = bb * 0x10 + 8;
		//GSE_LOG("%s: read (%d) buffer[8] = 0x%x\n", __func__, (buffer+8), *(buffer+8));
...
static int stk831x_gsensor_open_report_data(int open)
{
	//should queuq work to report event if  is_report_input_direct=true
	return 0;
}

static int stk831x_gsensor_enable_nodata(int value)
{
	struct stk831x_i2c_data *priv = obj_i2c_data;
	int err=0;
...
static int stk831x_gsensor_set_delay(u64 ns)
{
	u8 sample_delay;
	int err = 0;
	int value;	
...
static int stk831x_gsensor_get_data(int* x ,int* y,int* z, int* status)
{
	char buff[STK831X_BUFSIZE];
	struct stk831x_i2c_data *priv = obj_i2c_data;
	
	STK831X_ReadSensorData(priv->client, buff, STK831X_BUFSIZE);
...
static int stk831x_resume(struct i2c_client *client)
{
	...
	if(obj->re_enable == 1)
	{
		if(err = STK831X_SetPowerMode(obj->client, true))
		{
			GSE_ERR("stk831x_resume: write power control fail!!\n");
		//	return;
		}
		obj->re_enable = 0;
	}
...
//#if defined(MTK_AUTO_DETECT_ACCELEROMETER)
    if(err = stk831x_create_attr(&(stk831x_init_info.platform_diver_addr->driver)))
//#else
...
//	}
	...
	ctl.open_report_data= stk831x_gsensor_open_report_data;
	ctl.enable_nodata = stk831x_gsensor_enable_nodata;
	ctl.set_delay  = stk831x_gsensor_set_delay;
	...
	data.get_data = stk831x_gsensor_get_data;
...
static int stk831x_local_init(void)
{
    ...
    {
        GSE_ERR("stk831x prob and init error.\n");
        return -1;
    }
...
//    hwmsen_gsensor_add(&stk831x_init_info);
	acc_driver_add(&stk831x_init_info);
//#else
