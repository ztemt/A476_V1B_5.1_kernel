...
#ifdef CONFIG_COMPAT
static long lis3dh_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret, i;
	unsigned int tmp;
	void __user *arg32 = compat_ptr(arg);
#if 0
	tmp  = COMPAT_GSENSOR_IOCTL_SET_CALI;
	printk("COMPAT_GSENSOR_test____:%ul\n",tmp);
	tmp  = COMPAT_GSENSOR_IOCTL_GET_CALI;
	printk("COMPAT_GSENSOR_test____:%ul\n",tmp);
	tmp  = COMPAT_GSENSOR_IOCTL_CLR_CALI;
	printk("COMPAT_GSENSOR_test____:%ul\n",tmp);
	tmp  = COMPAT_GSENSOR_IOCTL_READ_SENSORDATA;
	printk("COMPAT_GSENSOR_test____:%ul\n",tmp);
#endif
	printk("yzb_lis3dh_compat_ioctlcmd:%ul   arg: 0x%lx, arg32: 0x%p\n",cmd,arg, arg32);
	...
	switch (cmd) {
		...
		case COMPAT_GSENSOR_IOCTL_READ_SENSORDATA:
									  ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_SENSORDATA,
																	  (unsigned long)arg32);  
													  break;
