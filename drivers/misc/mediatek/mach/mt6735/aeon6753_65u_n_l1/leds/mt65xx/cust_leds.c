...
int cust_set_button(unsigned int level)
{
	printk("cust_set_button level=%d\n",level);
	if(level == 0)
	{
		mt_set_gpio_mode(GPIO_LED_EN_PIN,GPIO_LED_EN_PIN_M_GPIO);  
	    mt_set_gpio_dir(GPIO_LED_EN_PIN,GPIO_DIR_OUT);
	    mt_set_gpio_out(GPIO_LED_EN_PIN,GPIO_OUT_ZERO);		
	}
	else
	{
		mt_set_gpio_mode(GPIO_LED_EN_PIN,GPIO_LED_EN_PIN_M_GPIO);  
	    mt_set_gpio_dir(GPIO_LED_EN_PIN,GPIO_DIR_OUT);
	    mt_set_gpio_out(GPIO_LED_EN_PIN,GPIO_OUT_ONE);
	}
}
...
static struct cust_mt65xx_led cust_led_list[MT65XX_LED_TYPE_TOTAL] = {
	...
	{"button-backlight",  MT65XX_LED_MODE_GPIO, cust_set_button,{0}},
