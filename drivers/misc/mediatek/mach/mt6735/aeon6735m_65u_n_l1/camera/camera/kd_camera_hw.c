...
void CAMERA_LDO_EN_fun(int flag)
{
	if(mt_set_gpio_mode(GPIO_CAMERA_LDO_EN_PIN,GPIO_CAMERA_LDO_EN_PIN_M_GPIO)){PK_DBG("[CAMERA LDO EN] set gpio mode failed!! (CAMERA_LDO)\n");}
	if(mt_set_gpio_dir(GPIO_CAMERA_LDO_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[CAMERA LDO_EN] set gpio dir failed!! (CAMERA_LDO)\n");}
	if(flag == 1){
		if(mt_set_gpio_out(GPIO_CAMERA_LDO_EN_PIN,GPIO_OUT_ONE)){PK_DBG("[CAMERA LDO_EN] set gpio failed!! (CAMERA_LDO)\n");}
	}else{
		if(mt_set_gpio_out(GPIO_CAMERA_LDO_EN_PIN,GPIO_OUT_ZERO)){PK_DBG("[CAMERA LDO_EN] set gpio failed!! (CAMERA_LDO)\n");}
	}	
}
...
u32 pinSet[3][8] = {
    ...
    if (On) {
        ...
        printk("[kd_camera_hw][PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, currSensorName);
 ...
 //VCAM_A
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW_SUB, currSensorName)))
	{
            //First Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
 ...
 //VCAM_A
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW_DL, currSensorName)))
	{
            //First Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
 ...
 //VCAM_A
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_S5K4H5YC_MIPI_RAW_SUB_DL, currSensorName)))
	{
            //First Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
 ...
 //VCAM_A
	 ...
	 mdelay(5);
		...
		else  if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_T4KA3OF_MIPI_RAW, currSensorName)))
		{
            //First Power Pin low and Reset Pin Low
           printk("[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, SENSOR_DRVNAME_T4KA3OF_MIPI_RAW);
			#if 1
			if(pinSetIdx ==1)
			{
			     printk("sub camera will be on in main checking\n");
				 return 0;
			}
			#endif
		...
		else  if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_T4KA3COS_MIPI_RAW, currSensorName)))
		{
            //First Power Pin low and Reset Pin Low
           printk("[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, SENSOR_DRVNAME_T4KA3COS_MIPI_RAW);
			#if 1
			if(pinSetIdx ==1)
			{
			     printk("sub camera will be on in main checking\n");
		...
		else  if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_T4KA3_MIPI_RAW, currSensorName)))
        {
            //First Power Pin low and Reset Pin Low
           printk("[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, SENSOR_DRVNAME_T4KA3_MIPI_RAW);
			#if 1
			if(pinSetIdx ==1)
			{
			     printk("sub camera will be on in main checking\n");
       ...
       if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
		...
		else  if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_T4KA3_MIPI_RAW_SUB, currSensorName)))
		{
            //First Power Pin low and Reset Pin Low
           printk("[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, SENSOR_DRVNAME_T4KA3_MIPI_RAW_SUB);
			#if 1
			if(pinSetIdx ==0)
			{
			     printk("main camera will be on in sub checking\n");
