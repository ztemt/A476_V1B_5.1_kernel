...
//	52, 52, 52, 52, 52, 52, 52, 52, 50, 43, 38, 33, 28, 20, 15, 10
	1, 1, 1, 1, 1, 2, 2, 2, 3, 3, 4, 5, 6, 8, 10, 13 //sum 64
};
static int disp_pwm_level_remap(disp_pwm_id_t id, int level_1024)
{
	...
	if(level_1024 > 1023) level_1024 = 1023;
		
	if(level_1024 > 0){
		level_1024 = level_1024/4; // 0-255
		idx = level_1024/16;
		offset = level_1024%16;
		
		level_1024 = 16;
		for(i=0; i<idx; i++) {
			level_1024 += (level_map[i]*16);
		}
		level_1024 += (level_map[idx]*(offset+1));
	}
	
	//printk("disp_pwm_level_remap: level_1024=%d\n", level_1024);
    return level_1024;
}
