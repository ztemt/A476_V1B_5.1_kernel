...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
		...
		UINT32 recv_data_cnt;
		UINT32 data_arr[4] = {0,0,0,0};
		static UINT32 data_paras[2] = {0x0f,0xf0};
		unsigned char packet_type;
	  ...
	  //DSI_OUTREGBIT(cmdq_trigger_handle, DSI_INT_ENABLE_REG,DSI_REG[dsi_i]->DSI_INTEN,RD_RDY,0);
		...
		else if(state == CMDQ_ESD_CHECK_CMP)
		{
			...
			{			
				...
				printk("[DSI]enter cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x,para_list[2]=0x%x\n",dsi_params->lcm_esd_check_table[i].cmd,dsi_params->lcm_esd_check_table[i].count,dsi_params->lcm_esd_check_table[i].para_list[0],dsi_params->lcm_esd_check_table[i].para_list[1],dsi_params->lcm_esd_check_table[i].para_list[2]);
