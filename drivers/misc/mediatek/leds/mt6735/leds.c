...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	static bool disable_hw_first_time = true;
	...
	mutex_lock(&leds_pmic_mutex);
		if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK0)
		{
			...
			if(disable_hw_first_time == true)
			{
				/* xuqian add for disable hardware control led */
				mt_set_gpio_mode(GPIO83,GPIO_MODE_00);  
				mt_set_gpio_dir(GPIO83,GPIO_DIR_OUT);
				mt_set_gpio_out(GPIO83,GPIO_OUT_ONE);
				printk("xuqian disable hardware control red led\n");
				disable_hw_first_time = false;
			}
			return 0;
		}
