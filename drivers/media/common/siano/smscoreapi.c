...
static int default_mode = SMSHOSTLIB_DEVMD_DVBT2;//4; //wood, ++--
...
#else
		sms_static_combuf = dma_alloc_coherent(siano_mm, PAGE_ALIGN(COMMOM_SIZE),
...
#ifdef SMS_TS_RECORD
void smscore_ts_file_open(void * dev, const char* path, int flags, int rights)
{
    ...
    if(IS_ERR(filp)) {
        err = PTR_ERR(filp);
		sms_err("TS file open error:%d", err);
		coredev->f = NULL;
    } else {
		coredev->f = filp;
	}
...
int smscore_ts_file_write(void * dev, unsigned long offset, unsigned int size)
{
	struct smscore_device_t *coredev = dev;
	mm_segment_t oldfs;
	int ret;
	...
	ret = vfs_write(coredev->f, &coredev->common_buffer[offset], size, &coredev->f->f_pos);

	set_fs(oldfs);
	return ret;
}

void smscore_ts_file_close(void * dev)
{
	struct smscore_device_t *coredev = dev;
	if (coredev->f) {
		filp_close(coredev->f, NULL);
		coredev->f = NULL;
	}
}
...
#ifdef SMS_TS_RECORD
	if (arrival)
		smscore_ts_file_open(coredev, RECORD_FILE, O_RDWR | O_CREAT,0644);
#endif
...
int smscore_register_device(struct smsdevice_params_t *params, void **coredev)
{
...
#else
		dev->common_buffer = dma_alloc_coherent(siano_mm, dev->common_buffer_size,
	...
	dev->standby_mode = SMSHOSTLIB_DEVMD_NONE;
...
#ifdef SMS_MULTI_FW_SUPPORTED
{
	...
	case SMSHOSTLIB_DEVMD_DVBT:
		_standby_mode = SMSHOSTLIB_DEVMD_DVBT2;
		break;

	case SMSHOSTLIB_DEVMD_DVBT2:
		_standby_mode = SMSHOSTLIB_DEVMD_DVBT;
		break;
		
	default:
		_standby_mode = SMSHOSTLIB_DEVMD_NONE;
		break;
	}

	sms_info("set device standby mode to %d", _standby_mode);

	if (_standby_mode != SMSHOSTLIB_DEVMD_NONE)
		smscore_set_device_standby_mode(coredev, _standby_mode);
}
#endif	
#endif
	...
	schedule_delayed_work(&coredev->work_queue, 0);//msecs_to_jiffies(100)
#ifdef SMS_RC_SUPPORT_SUBSYS
					 ...
					 void *buffer, size_t size)
{
			...
			(DownloadAttempts < MAX_FW_DOWNLOAD_ATTEMPTS) )
	{
		...
		if (calc_checksum == coredev->fw_checkSum)
		{
			...
			sms_err("MSG_SMS_DATA_VALIDITY_RES, checksum = 0x%x", coredev->fw_checkSum);
		}
		DownloadAttempts++;
	}
...
#ifdef SMS_MULTI_FW_SUPPORTED
	if ((mode == coredev->standby_mode) && (coredev->standby_mode != SMSHOSTLIB_DEVMD_NONE))
	{
		struct SmsMsgHdr_S *msg;
		...
		SMS_INIT_MSG(msg, MSG_SW_SWITCH_STANDARD_REQ,
				 sizeof(struct SmsMsgHdr_S));	
		smsendian_handle_tx_message((struct SmsMsgHdr_S *)msg);
	
		//wait MSG_SMS_DEVICE_DETECTED_IND msg
		init_completion(&coredev->device_ready_done);
		rc = coredev->sendrequest_handler(coredev->context, msg, msg->msgLength);
		if (rc < 0) {				
			sms_err("send MSG_SW_SWITCH_STANDARD_REQ failed, rc %d", rc);
			kfree(msg);
			return rc;
		}	

		//the MSG_SMS_DEVICE_DETECTED_IND msg cost about 600ms
		rc = wait_for_completion_timeout(&coredev->device_ready_done,
					msecs_to_jiffies(1000)) ?
					0 : -ETIME;
		if (rc < 0)
			sms_err("device switch standard failed, rc %d", rc);
		...
		if (rc < 0) 
			...
			{
			case SMSHOSTLIB_DEVMD_DVBT:
				coredev->standby_mode = SMSHOSTLIB_DEVMD_DVBT2;
				break;

			case SMSHOSTLIB_DEVMD_DVBT2:
				coredev->standby_mode = SMSHOSTLIB_DEVMD_DVBT;
				break;
				
			default:
				coredev->standby_mode = SMSHOSTLIB_DEVMD_NONE;
				break;
			}
		}
...
static int smscore_load_standby_firmware(struct smscore_device_t *coredev, int mode)
{
	struct SmsFirmware_ST *firmware;
	struct SmsMsgHdr_S *msg;
	u32 mem_address,  calc_checksum = 0;
	u32 i, *ptr;
	u32 FwSize;
	...
	struct sms_firmware_t *sms_standby_firmware = NULL;
	const struct firmware *fw;
	char* fw_filename = smscore_get_fw_filename(coredev, mode, 0);

	sms_info("loading standby firmware:%s", fw_filename);
	if (!strcmp(fw_filename,"none"))
		return -ENOENT;

	if (coredev->loadfirmware_handler == NULL && !(coredev->device_flags
			& SMS_DEVICE_FAMILY2))
		return -EINVAL;
	...
	sms_info("read standby fw %s, buffer size=0x%x", fw_filename, fw->size);
	sms_standby_firmware = kmalloc(ALIGN(fw->size + sizeof(*sms_standby_firmware), SMS_ALLOC_ALIGNMENT),
			GFP_KERNEL | GFP_DMA);
	if (!sms_standby_firmware) {
		sms_info("failed to allocate standby firmware buffer");
		return -ENOMEM;
	}
	sms_info("buffer 0x%p allocated. copy filename", sms_standby_firmware);
	memcpy(sms_standby_firmware->file_name, fw_filename, sizeof(sms_standby_firmware->file_name));
	sms_info("copy data");
	memcpy(sms_standby_firmware->data, fw->data, fw->size);
	sms_standby_firmware->fw_buf_size = fw->size;
	sms_info("release the fw.");
	release_firmware(fw);

	firmware = (struct SmsFirmware_ST *)sms_standby_firmware->data;
	FwSize = sms_standby_firmware->fw_buf_size;

#else

	if (!coredev->firmware) {
			sms_info("missing standby fw file buffer");
			return -EINVAL;
		}


	firmware = (struct SmsFirmware_ST *)coredev->firmware->data;
	FwSize = coredev->firmware->fw_buf_size;

#endif	

	firmware->StartAddress = le32_to_cpu(0xe00000);
	firmware->Length = le32_to_cpu(firmware->Length);

	sms_info("loading FW to addr 0x%x size %d",
		 firmware->StartAddress, firmware->Length);
	...
	if (mode != SMSHOSTLIB_DEVMD_NONE) {
		sms_debug("send 'reload init' command.");
		SMS_INIT_MSG(msg, MSG_SW_RELOAD_START_REQ,
			     sizeof(struct SmsMsgHdr_S) + sizeof(u32));
		((struct SmsMsgData_S *)msg)->msgData[0] = 12;
	...
	if (coredev->fw_checkSum != calc_checksum)
	{
		sms_debug("the checksum is wrong:%d\n", coredev->fw_checkSum);
		goto exit_standby_fw_download;
	}
...
exit_standby_fw_download:
	sms_err("Exit fw download: rc=%d, postload=0x%p ", rc, coredev->postload_handler);
	kfree(msg);
#ifdef REQUEST_FIRMWARE_SUPPORTED
	if (sms_standby_firmware)
		kfree(sms_standby_firmware);
#endif
	return rc;
}

int smscore_set_device_standby_mode(void* dev, int mode)
{
	int rc = 0;
	struct smscore_device_t *coredev = (struct smscore_device_t *)dev;

	if (coredev->standby_mode == mode) {
		sms_err("standby mode %d already set", mode);
		return rc;
	}
	...
	if (mode == SMSHOSTLIB_DEVMD_DVBT || mode == SMSHOSTLIB_DEVMD_DVBT2)
		coredev->standby_mode = mode;
	else
		coredev->standby_mode = SMSHOSTLIB_DEVMD_NONE;

	sms_debug("set device standby mode to %d", coredev->standby_mode);

	if (coredev->standby_mode != SMSHOSTLIB_DEVMD_NONE)
 		rc = smscore_load_standby_firmware(coredev, coredev->standby_mode);

	return rc;
}
EXPORT_SYMBOL_GPL(smscore_set_device_standby_mode);
...
void smscore_onresponse(void* dev, struct smscore_buffer_t *cb) 
{
	...
	static unsigned long data_total; /* = 0; */
	unsigned long time_now = jiffies_to_msecs(jiffies);

	if (!last_sample_time)
		last_sample_time = time_now;

	if (time_now - last_sample_time > 3000) {
	...
	if (coredev->standby_mode != SMSHOSTLIB_DEVMD_NONE)
	{
		...
		case MSG_SW_SWITCH_STANDARD_RES:
			//complete(&coredev->reload_start_done);
			break;

		case MSG_SMS_DEVICE_DETECTED_IND:
			  ...
			  void *buffer, size_t size)
{
	...
	struct SmsMsgData_S *tempphdr = (struct SmsMsgData_S *) buffer;
	int rc;
...
#if 1
{
	...
	tempbuffer = kmalloc(PAGE_SIZE*8, GFP_KERNEL | GFP_DMA);
	
	if (!tempbuffer) {
		sms_err("Could not allocate buffer for sending message!");
		return -ENOMEM;
	}
	
	msg = (struct SmsMsgData_S *)SMS_ALIGN_ADDRESS(tempbuffer);
	SMS_INIT_MSG(&msg->xMsgHeader, phdr->msgType,
			sizeof(struct SmsMsgData_S)+sizeof(u32)*5);

	msg->xMsgHeader.msgSrcId= phdr->msgSrcId;
	msg->xMsgHeader.msgDstId= phdr->msgDstId;
	msg->xMsgHeader.msgFlags= phdr->msgFlags;
	msg->msgData[0] =tempphdr->msgData[0];
	msg->msgData[1] =tempphdr->msgData[1];
	msg->msgData[2] =tempphdr->msgData[2];
	msg->msgData[3] =tempphdr->msgData[3];
	msg->msgData[4] =tempphdr->msgData[4];

	return coredev->sendrequest_handler(coredev->context, msg, msg->xMsgHeader.msgLength);
}
#endif
...
static int __init smscore_module_init(void)
{
	 ...
	 */
	 printk("111\n");
	siano_mm=kzalloc(sizeof(struct device), GFP_KERNEL);
	dev_set_name(siano_mm, "siano_tp");
	printk("llalal\n");
	device_initialize(siano_mm);
	if(siano_mm == NULL)
		printk("22222\n");
	printk("333333\n");
