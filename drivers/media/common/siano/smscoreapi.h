...
//#define SMS_TS_RECORD //wood, ++, record ts data in kernel level
#define RECORD_FILE "out.ts"
...
#if 0//defined(CONFIG_FW_LOADER) || (defined(CONFIG_FW_LOADER_MODULE) && defined(MODULE))
...
#ifdef SMS_TS_RECORD
extern void smscore_ts_file_open(void *coredev, const char* path, int flags, int rights);
extern int smscore_ts_file_write(void *coredev, unsigned long offset, unsigned int size);
extern void smscore_ts_file_close(void *coredev);
...
extern int smscore_set_device_standby_mode(void *coredev, int mode);
...
#ifdef CONFIG_MEDIATEK_SOLUTION /* CONFIG_ARCH_ ... */
#define SDIO_PORT_NUM  2
extern int sms_sdio_ctrl (unsigned int sdio_port_num, unsigned int on);
#define sms_sdio_plug_notify(id,plugin) sms_sdio_ctrl (SDIO_PORT_NUM,plugin)

#elif defined(CONFIG_PLAT_RK)

extern int rk29sdk_wifi_set_carddetect(int val);

#define sms_sdio_plug_notify(id,plugin) rk29sdk_wifi_set_carddetect(plugin)

#elif defined(CONFIG_ARCH_MSM)

#define SDIO_PORT_NUM  1
extern int sms_sdio_ctrl (unsigned int sdio_port_num, unsigned int on);
#define sms_sdio_plug_notify(id,plugin) sms_sdio_ctrl (SDIO_PORT_NUM,plugin)
