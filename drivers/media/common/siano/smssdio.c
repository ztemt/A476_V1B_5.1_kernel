...
static void smssdio_work_thread(struct work_struct *arg)
{
	...
	if (size) {
		...
		if (ret) {
				...
				cb = NULL;
				abort = 0x1;
				sdio_writeb(&smsdev->sdio_func_0, abort, SMSSDIO_CCCR, &ret);
				goto exit_with_error;
			}
	}
			 ...
			 const struct sdio_device_id *id)
{
	...
	if (func->num == 1)
	{
		smsdev->sdio_func_0.num=0;
		smsdev->sdio_func_0.card = func->card;
	}
	...
	params.num_buffers = MAX_SDIO_BUF_NUMBER;
