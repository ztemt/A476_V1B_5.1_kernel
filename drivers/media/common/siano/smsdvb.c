...
struct smsdvb_frontend_t{
	...
	int tuner_id;
	struct STATISTIC_INFO_S statistic_info;
...
static void smsdvb_update_dvb_stats(struct STATISTIC_INFO_S *pStatisticInfo,
				   struct SMSHOSTLIB_STATISTICS_S *p)
{
	...
	pStatisticInfo->IsRfLocked = p->IsRfLocked;
	pStatisticInfo->IsDemodLocked = p->IsDemodLocked;
	pStatisticInfo->SNR = p->SNR;
	pStatisticInfo->BER = p->BER;
	pStatisticInfo->InBandPwr = p->InBandPwr;

};


static void smsdvb_update_isdbt_stats(struct STATISTIC_INFO_S *pStatisticInfo,
				    struct SMSHOSTLIB_STATISTICS_ISDBT_S *p)
{
	...
	pStatisticInfo->IsDemodLocked = p->IsDemodLocked;
	pStatisticInfo->SNR = p->SNR;
	pStatisticInfo->BER = p->LayerInfo[0].BER +
  			      p->LayerInfo[1].BER +
			      p->LayerInfo[2].BER ;
	pStatisticInfo->PER = p->LayerInfo[0].TS_PER +
			      p->LayerInfo[1].TS_PER + 
			      p->LayerInfo[2].TS_PER;
	pStatisticInfo->InBandPwr = p->InBandPwr;
}
...
static void smsdvb_update_dvbt2_stats(struct STATISTIC_INFO_S *pStatisticInfo,
				    struct SMSHOSTLIB_STATISTICS_DVBT2_S *p)
{
	sms_debug("IsModemLocked = %d", p->ReceptionData.IsModemLocked);
	sms_debug("SNR = %d", p->ReceptionData.SNR);
	sms_debug("InBandPwr = %d", p->ReceptionData.inbandPower);
	sms_debug("BER = %d", p->ReceptionData.FER);

	/* update reception data */
	pStatisticInfo->IsRfLocked = p->ReceptionData.IsModemLocked;
	pStatisticInfo->IsDemodLocked = p->ReceptionData.IsModemLocked;
	pStatisticInfo->SNR = p->ReceptionData.SNR;
	pStatisticInfo->BER = p->ReceptionData.FER;
	pStatisticInfo->PER = p->activePlps[0].totalNumErrTsPktsReceived + 
			      p->activePlps[1].totalNumErrTsPktsReceived;
	pStatisticInfo->InBandPwr = p->ReceptionData.inbandPower;
...
static int smsdvb_onresponse(void *context, struct smscore_buffer_t *cb)
{
	...
	case MSG_SMS_SIGNAL_DETECTED_IND:
		...
		sms_frontend->statistic_info.IsDemodLocked = true;
		is_status_update = true;
		break;

	case MSG_SMS_NO_SIGNAL_IND:
		sms_info("MSG_SMS_NO_SIGNAL_IND");
		sms_frontend->statistic_info.IsDemodLocked = false;
		is_status_update = true;
		break;
	...
	case MSG_SMS_TRANSMISSION_IND: {
		...
		/* update reception data */
		sms_frontend->statistic_info.IsDemodLocked = pTrnsInd->IsDemodLocked;
		is_status_update = true;
		break;
	}
	case MSG_SMS_HO_PER_SLICES_IND: {
		sms_info("MSG_SMS_HO_PER_SLICES_IND");
		sms_frontend->statistic_info.IsRfLocked = pMsgData[16];
		sms_frontend->statistic_info.IsDemodLocked = pMsgData[17];
		sms_frontend->statistic_info.SNR = pMsgData[1];
		sms_frontend->statistic_info.BER = pMsgData[13];

		sms_frontend->statistic_info.InBandPwr = (s32) pMsgData[2];
		is_status_update = true;
		break;
	}
	case MSG_SMS_GET_STATISTICS_RES: {
		...
		is_status_update = true;
		smsdvb_update_dvb_stats(&sms_frontend->statistic_info, p_dvb_stats);
		complete(&sms_frontend->get_stats_done);
		break;
	}
	case MSG_SMS_GET_STATISTICS_EX_RES: {
		struct sms_properties_t properties;		
		sms_info("MSG_SMS_GET_STATISTICS_EX_RES status=0x%x", *pMsgData);
		    ...
		    properties.mode == SMSHOSTLIB_DEVMD_DVBT_BDA)
		{
			...
			//struct SMSHOSTLIB_STATISTICS_DVBT_S *p_dvbt_stats = (void *)(pMsgData+1);
			sms_info("Recieved statistics of DVBT ****** May be a problem");
			//smsdvb_update_dvbt_stats_ex(&sms_frontend->statistic_info, p_dvbt_stats);
		}
		else if (properties.mode == SMSHOSTLIB_DEVMD_DVBT2 || 
		    	 properties.mode == SMSHOSTLIB_DEVMD_DVBT2_BDA)
		{
			struct SMSHOSTLIB_STATISTICS_DVBT2_S  *p_dvbt2_stats = (void *)(pMsgData+1);
			sms_info("Recieved statistics of DVBT2");
			smsdvb_update_dvbt2_stats(&sms_frontend->statistic_info, p_dvbt2_stats);
		}
		else if (properties.mode == SMSHOSTLIB_DEVMD_ISDBT || 
		    	 properties.mode == SMSHOSTLIB_DEVMD_ISDBT_BDA)
		{
			struct SMSHOSTLIB_STATISTICS_ISDBT_S  *p_isdbt_stats = (void *)(pMsgData+1);
			sms_info("Recieved statistics of ISDBT");
			smsdvb_update_isdbt_stats(&sms_frontend->statistic_info, p_isdbt_stats);
		}		
		is_status_update = true;		   
		complete(&sms_frontend->get_stats_done);
		break;
	...
	case MSG_SMS_SLAVE_DEVICE_DETECTED:
		sms_info("MSG_SMS_SLAVE_DEVICE_DETECTED");
		break;

	case MSG_SMS_ADD_PID_FILTER_RES:
		sms_info("MSG_SMS_ADD_PID_FILTER_RES");
		break;
	case MSG_SMS_REMOVE_PID_FILTER_RES:
		sms_info("MSG_SMS_REMOVE_PID_FILTER_RES");
		break;
...
static int smsdvb_get_statistics_ex(struct dvb_frontend *fe) 
{
	...
	smscore_get_device_properties(client->coredev, &properties);
	if (properties.board_id == SMS1XXX_BOARD_SIANO_SIENA &&
	    (properties.mode == SMSHOSTLIB_DEVMD_DVBT || 
	    properties.mode == SMSHOSTLIB_DEVMD_DVBT_BDA ||
...
static inline int led_feedback(struct smsdvb_frontend_t* sms_frontend)
{
	if (sms_frontend->fe_status & FE_HAS_LOCK)
	{
		return sms_board_led_feedback(sms_frontend->client->coredev,
				(sms_frontend->statistic_info.BER== 0) ? SMS_LED_HI : SMS_LED_LO);
	}
...
static int smsdvb_read_ber(struct dvb_frontend *fe, u32 *ber)
{
	...
	rc = smsdvb_get_statistics_ex(fe);
	*ber = sms_frontend->statistic_info.BER;
	led_feedback(sms_frontend);
	return rc;
}
...
static int smsdvb_read_signal_strength(struct dvb_frontend *fe, u16 *strength)
{
	...
	rc = smsdvb_get_statistics_ex(fe);
	if (sms_frontend->statistic_info.InBandPwr < -95)
		*strength = 0;
		else if (sms_frontend->statistic_info.InBandPwr > -29)
			*strength = 100;
		else
			*strength =
				(sms_frontend->statistic_info.InBandPwr
				+ 95) * 3 / 2;
	led_feedback(sms_frontend);
	return rc;
}
...
static int smsdvb_read_snr(struct dvb_frontend *fe, u16 *snr)
{
	...
	rc = smsdvb_get_statistics_ex(fe);
	*snr = sms_frontend->statistic_info.SNR;
	led_feedback(sms_frontend);
	return rc;
}
...
static int smsdvb_read_ucblocks(struct dvb_frontend *fe, u32 *ucblocks)
{
	...
	rc = smsdvb_get_statistics_ex(fe);
	*ucblocks = sms_frontend->statistic_info.PER;
	led_feedback(sms_frontend);
	return rc;
}
...
static int smsdvb_dvbt_set_frontend(struct dvb_frontend *fe)
{
	...
	sms_info("setting DVB freq to %d BW %d on tuner %d", c->frequency, c->bandwidth_hz, sms_frontend->tuner_id);
...
#else
static int smsdvb_set_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters* p_params)
#endif

{
	...
	case SMSHOSTLIB_DEVMD_DVBT2_BDA:
...
#else
static int smsdvb_get_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters* p_params)
#endif
{
	struct smsdvb_frontend_t* sms_frontend = container_of(fe, struct smsdvb_frontend_t, fe);
...
int smsdvb_set_property(struct dvb_frontend *fe, struct dtv_property *tvp) 
{
	...
	if (tvp->cmd ==  DTV_DELIVERY_SYSTEM)
	{
		...
		{
		case SYS_DVBT:
			sms_debug ("smsdvb_set_property: Requested DVBT");
			request_mode = SMSHOSTLIB_DEVMD_DVBT_BDA;
			break;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0)
		case SYS_DVBT2:
			sms_debug ("smsdvb_set_property: Requested DVBT2");
			request_mode = SMSHOSTLIB_DEVMD_DVBT2_BDA;
			break;
#endif
		case SYS_DVBH:
			...
			break;
		case SYS_ISDBT:
			sms_debug ("smsdvb_set_property: Requested ISDBT");
...
/*Mode change is currently nots supported until players will configure the mode properly*/
		...
		if (smscore_set_device_mode(sms_frontend->client->coredev, request_mode))
		{
			sms_err ("Setting device mode to %d failed!", request_mode);
			return -EOPNOTSUPP;
		}
		sms_debug ("Wait after setting mode for slave detection");
		msleep(500);
		PidMsg.xMsgHeader.msgSrcId = DVBT_BDA_CONTROL_MSG_ID;
		PidMsg.xMsgHeader.msgDstId = hif_task_of_device[sms_frontend->tuner_id];
		PidMsg.xMsgHeader.msgFlags = 0;
		PidMsg.xMsgHeader.msgType  = MSG_SMS_REMOVE_PID_FILTER_REQ;
		PidMsg.xMsgHeader.msgLength = sizeof(PidMsg);
		PidMsg.msgData[0] = 0x2000;

		smsendian_handle_tx_message((struct SmsMsgHdr_S *)&PidMsg);
		return smsclient_sendrequest(sms_frontend->client->smsclient,
					     &PidMsg, sizeof(PidMsg));
