...
#define HOST_INTERRUPT_PIN 	RK30_PIN0_PA7//135
...
#define SMSI2C_CLIENT_ADDR		0x77//0x68
...
/*******************************************************************/
#define I2C_SCL_TATE (1000 *1000) //(400*1000)
...
static void smsi2c_interrupt(int irq, void *context)
{
...
#ifdef SMS_RK_TS
#define MAX_I2C_BUF_SIZE	MAX_BUF_SIZE
#define MAX_I2C_BUF_NUMBER	MAX_BUF_NUMBER
...
#ifdef SMS_RK_TS
extern void rk_ts_callback_setup(void * host , \
	int callback(void *args, unsigned char * ts_buffer, int size), void * data);
extern int rk29_hsadc_start_transmit(int polling_time);//in msec unit
...
int smsi2c_register(void)
{
	...
	if (!ret) {
		/*********************
		* start rk ts interface
		* ts_set_callback(void *, smsi2c_ts_feed, smsdev->coredev);
		************************/
		rk_ts_callback_setup(NULL, smsi2c_ts_feed, g_smsi2c_device->coredev);
		rk29_hsadc_start_transmit(20);
	}
#endif
