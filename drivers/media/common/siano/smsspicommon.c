				...
				int padding_allowed)
{
	...
	while (bytes_to_transfer || unused_bytes) {
		if ((unused_bytes <= 0) && (bytes_to_transfer > 0)) {
			...
			if ((len + offset) > RX_BUFFER_SIZE) {
				PRN_ERR((TXT("too much data to tx: %d > buffer size: %d\n"), (len + offset), RX_BUFFER_SIZE));
			}
		...
		if (bytes_to_transfer < missing_bytes) {
			bytes_to_transfer =
			    (((missing_bytes + SPI_PACKET_SIZE -
			     ...
			     SPI_PACKET_SIZE_BITS);
			PRN_DBG((TXT
	("a message was found, adding bytes to transfer, txmsg %p, tx_bytes %d, total %d, unused_bytes %d, missing_bytes %d\n")
			, txmsg, tx_bytes, bytes_to_transfer, unused_bytes, missing_bytes));
		}
	}
	dev->cb.free_rx_buf(dev->context, buf);
}
