...
static struct mutex g_smschar_socketslock;

int smschar_dev_socket_id(int dev)
{
	int base = MKDEV(smschar_major, smschar_minor);
	return (dev - base) / SMSCHAR_NR_NODES;
}

int smschar_dev_index(int dev)
{
	int base = MKDEV(smschar_major, smschar_minor);
	return (dev - base) % SMSCHAR_NR_NODES;
}
...
int smschar_dev_is_ctrl(int dev)
{
	int base = MKDEV(smschar_major, smschar_minor);
	return !((dev - base) % SMSCHAR_NR_NODES);
}

#ifdef SMS_POWER_CTL_OPEN
static int smschar_getdev(struct smschar_device_t *dev)
{
	int times = 100;
	int count;
	int socket_id = smschar_dev_socket_id(dev->cdev.dev);
	struct smschar_socket_t *socket;
	socket = &g_smschar_sockets[socket_id];

	kmutex_lock(&g_smschar_socketslock);
	if (dev->coredev) {
		if (socket->removing) {
			kmutex_unlock(&g_smschar_socketslock);
			sms_debug("socket: %d is removing.\n", socket_id);
			return -EAGAIN;
		} else {
			atomic_inc(&socket->count);
			kmutex_unlock(&g_smschar_socketslock);
			return 0;
		}
		...
		count = atomic_inc_return(&socket->count);
		if (1 == count) {
			sms_chip_poweron(socket_id);
			sms_chip_plug(socket_id, 1);
		}

		while (times-- && !dev->coredev)
			msleep(50);
		...
		count = atomic_dec_return(&socket->count);

		if (0 == count) {
			sms_chip_poweroff(socket_id);
			sms_chip_plug(socket_id, 0);
		}
		return -ENODEV;
...
static int smschar_putdev(struct smschar_device_t *dev)
{
	int count;
	int times = 100;
	int socket_id = smschar_dev_socket_id(dev->cdev.dev);
	...
	sms_debug("removing socket: %d.\n", socket_id);

	sms_chip_plug(socket_id, 0);

	while (times-- && dev->coredev) {
		sms_debug("sms chip wait for plug off times:%d", times);
		msleep(50);
	}

	if (dev->coredev) {
		sms_err("sms chip plug off error!\n");
		return 0;
	}
	sms_debug("removed socket: %d.\n", socket_id);
	sms_chip_poweroff(socket_id);
	socket->removing = 0;
	return 0;
}
...
int smschar_send_fw_file(struct smschar_device_t *dev,
	...
	return smscore_send_fw_file(dev->coredev, up->fw_buf, up->fw_size);
}
...
static int smschar_flush_buffer(struct smschar_device_t *dev)
{
	unsigned long flags;
	...
	if (cb) {
		...
		sms_debug("flush current buffer.\n");
	}
	...
	while (!list_empty(&dev->pending_data)) {
		...
		if (!list_empty(&dev->pending_data)) {
			cb = (struct smscore_buffer_t *)dev->pending_data.next;
			list_del(&cb->entry);
			dev->pending_count--;
		}
		spin_unlock_irqrestore(&dev->lock, flags);
		if (cb) {
			smscore_putbuffer(dev->coredev, cb);
			sms_debug("flush pending buffer.\n");
		}
	}
...
static int smschar_onresponse(void *context, struct smscore_buffer_t *cb)
{
	...
	if(dev->cancel_waitq) {
		sms_debug("cancel_waitq, not to pend.\n");
		return -EFAULT;
	}
	spin_lock_irqsave(&dev->lock, flags);
...
static int smschar_open(struct inode *inode, struct file *file)
{
	struct smschar_device_t *dev = container_of(inode->i_cdev,
						    struct smschar_device_t,
						    cdev);
	int count = atomic_inc_return(&dev->count);
	int socket_id = smschar_dev_socket_id(dev->cdev.dev);
	sms_info("socket: %d, index: %d, count: %d\n", socket_id, smschar_dev_index(dev->cdev.dev), count);

#ifdef SMS_POWER_CTL_OPEN
	{
		int err = smschar_getdev(dev);
		if (err) {
...
static int smschar_release(struct inode *inode, struct file *file)
{
	...
	int count = atomic_dec_return(&dev->count);
	int socket_id = smschar_dev_socket_id(dev->cdev.dev);
			 ...
			 unsigned int cmd, unsigned long arg)
{
	...
	int socket_id = smschar_dev_socket_id(dev->cdev.dev);
	//sms_debug("unlocked_socket: %d, index: %d, cmd: 0x%x, arg: %p\n", socket_id, smschar_dev_index(dev->cdev.dev), cmd, up);
	switch (cmd) {
	case SMSCHAR_IS_DEVICE_PNP_EVENT:   // PASS
		{
			printk("in smschar_ioctl SMSCHAR_IS_DEVICE_PNP_EVENT\n");
	...
	case SMSCHAR_IS_CANCEL_DEVICE_PNP_EVENT:     // PASS
		{
			g_smschar_pnp_event = SMSCHAR_TERMINATE_EVENT;
			g_pnp_status_changed = 1;
			printk("in smschar_ioctl SMSCHAR_IS_CANCEL_DEVICE_PNP_EVENT\n");
	...
	case SMSCHAR_SET_DEVICE_MODE:   
		printk("in smschar_ioctl SMSCHAR_SET_DEVICE_MODE\n");  //OK
		return smscore_set_device_mode(dev->coredev, (int)arg);

	case SMSCHAR_SET_DEVICE_STANDBY_MODE:
		printk("in smschar_ioctl SMSCHAR_SET_DEVICE_STANDBY_MODE return 0\n");  //OK
		return smscore_set_device_standby_mode(dev->coredev, (int)arg);
		
	case SMSCHAR_GET_DEVICE_MODE:              //PASS
		{
			struct sms_properties_t properties;
			printk("in smschar_ioctl SMSCHAR_GET_DEVICE_MODE\n");  //OK
	...
	case SMSCHAR_GET_BUFFER_SIZE:                   //PASS
		{
			struct sms_properties_t properties;
			printk("in smschar_ioctl SMSCHAR_GET_BUFFER_SIZE\n");  //OK
			smscore_get_device_properties(dev->coredev, &properties);
			if (put_user
...
#ifdef SMS_TS_RECORD
			if (dev->device_index) {
				smscore_ts_file_write(dev->coredev, touser.offset, touser.size);
			}
#endif
	...
	case SMSCHAR_CANCEL_WAIT_BUFFER:            // ++
		{
			printk("in smschar_ioctl SMSCHAR_CANCEL_WAIT_BUFFER\n");  //OK
			dev->cancel_waitq = 1;
			wake_up_interruptible(&dev->waitq);
			break;
		}
	...
	case SMSCHAR_GET_FW_FILE_NAME:
		{
			printk("in smschar_ioctl SMSCHAR_GET_FW_FILE_NAME\n"); 
			if (!up)
				return -EINVAL;
	...
	case SMSCHAR_SEND_FW_FILE:
		{
			printk("in smschar_ioctl SMSCHAR_SEND_FW_FILE\n"); 
			if (!up)
				return -EINVAL;
	...
	case SMSCHAR_SEND_FW_CHUNK:
		{
			printk("in smschar_ioctl SMSCHAR_SEND_FW_CHUNK\n"); 
			if (!up)
				return -EINVAL;
	...
	case SMSCHAR_SEND_LAST_FW_CHUNK:
		{
			printk("in smschar_ioctl SMSCHAR_SEND_LAST_FW_CHUNK\n"); 
			if (!up)
				return -EINVAL;
	...
	case SMSCHAR_RESET_GPIO_VALUE_CHANGE:                    // PASS
		{
			printk("in smschar_ioctl SMSCHAR_RESET_GPIO_VALUE_CHANGE\n"); 
...
#ifdef CONFIG_COMPAT
static long smschar_compat_ioctl(struct file *file,
			 unsigned int cmd, unsigned long arg)
{
	struct smschar_device_t *dev = file->private_data;
	void __user *up = (void __user *)arg;
	...
	//sms_debug("smschar_compat_ioctl: %d, index: %d, cmd: 0x%x, arg: %p\n", socket_id, smschar_dev_index(dev->cdev.dev), cmd, up);
	...
	case SMSCHAR_IS_DEVICE_PNP_EVENT:
		{
			printk("in smschar_compat_ioctl SMSCHAR_IS_DEVICE_PNP_EVENT\n");
			compat_int_t * pnpevent = up;
			sms_info("Waiting for PnP event.\n");
			printk("in SMSCHAR_IS_DEVICE_PNP_EVENT\n");
			wait_event_interruptible(g_pnp_event,
						 g_pnp_status_changed);
			sms_info("PnP event: g_pnp_status_changed=%d\n", g_pnp_status_changed);
			g_pnp_status_changed = 0;
			sms_info("PnP Event %d.\n", g_smschar_pnp_event);
			if (put_user(g_smschar_pnp_event, pnpevent))
			{
				sms_info("put_user returned error status\n");
				return -EFAULT;
			}
	...
	case SMSCHAR_IS_CANCEL_DEVICE_PNP_EVENT:
		{
			printk("in smschar_compat_ioctl SMSCHAR_IS_CANCEL_DEVICE_PNP_EVENT\n");     //test  1
	...
	case SMSCHAR_GET_DEVICE_MODE: // ok
		{
			struct sms_properties_t properties;
			compat_int_t * mode = up;
			printk("in smschar_compat_ioctl SMSCHAR_GET_DEVICE_MODE\n"); 
			smscore_get_device_properties(dev->coredev, &properties);
			if (put_user(properties.mode,
				     mode))
				return -EFAULT;
	...
	case SMSCHAR_GET_BUFFER_SIZE:
		{
			...
			struct sms_properties_t properties;
			compat_int_t * size = up;
			printk("in smschar_compat_ioctl SMSCHAR_GET_BUFFER_SIZE\n");   
		
			smscore_get_device_properties(dev->coredev, &properties);
			if (put_user
			    (smscore_get_common_buffer_size(dev->coredev), size))
				return -EFAULT;
			printk("end SMSCHAR_GET_BUFFER_SIZE\n");
		
			break;
		}

	case SMSCHAR_WAIT_GET_BUFFER_COMPAT:
		{
			struct smschar_buffer_t touser;
			struct smschar_compat_buffer_t user;
			int rc;
			
			rc = smschar_wait_get_buffer(dev, &touser);
			if (rc < 0)
				return rc;
			user.offset = touser.offset;
			user.size = touser.size;
			...
			if (copy_to_user(up, &user,
					 sizeof(struct smschar_compat_buffer_t)))
				return -EFAULT;

			break;
		}
	case SMSCHAR_SEND_FW_FILE_COMPAT:                       // test ok
		{
			struct smschar_send_fw_file_compat_ioctl_t * user_compat = up;
			struct smschar_send_fw_file_ioctl_t user;
			if (!up)
				return -EINVAL;
			user.fw_size = user_compat->fw_size;
			user.fw_buf = compat_ptr(user_compat->fw_buf);
 			return smschar_send_fw_file(dev, &user);
			break;
		}
	#if 0
	case SMSCHAR_GET_FW_FILE_NAME:
		{
			struct smschar_get_fw_filename_compat_ioctl_t * user_compat = up;
			struct smschar_get_fw_filename_ioctl_t user;
			printk("in SMSCHAR_GET_FW_FILE_NAME\n");
			if (!up)
				return -EINVAL;
			user.mode = user_compat->mode;
			user.filename = compat_ptr(user_compat->filename);
			
			return smschar_get_fw_filename(dev,&user);
			
		}
	#endif
	
	case SMSCHAR_SEND_FW_CHUNK_COMPAT:
		{
			struct smschar_send_fw_chunk_compat_ioctl_t * user_compat = up;
			struct smschar_send_fw_chunk_ioctl_t user;

			printk("in smschar_compat_ioctl SMSCHAR_SEND_FW_CHUNK_COMPAT\n");   //test 3
	
			
			if (!user_compat)
				return -EINVAL;
			
			user.size = user_compat->size;
			user.buf = compat_ptr(user_compat->buf);
			return smschar_send_fw_chunk(dev, &user);
			break;
		}
	case SMSCHAR_SEND_LAST_FW_CHUNK_COMPAT:
		{
			struct smschar_send_fw_chunk_compat_ioctl_t * user_compat = up;
			struct smschar_send_fw_chunk_ioctl_t user;
			printk("in smschar_compat_ioctl SMSCHAR_SEND_LAST_FW_CHUNK_COMPAT\n");
			if (!user_compat)
				return -EINVAL;
			user.size = user_compat->size;
			user.buf = compat_ptr(user_compat->buf);
			
 			return smschar_send_last_fw_chunk(dev, &user);
			break;
		}
	case SMSCHAR_RESET_GPIO_VALUE_CHANGE:
		{
			...
			printk("in smschar_compat_ioctl SMSCHAR_RESET_GPIO_VALUE_CHANGE\n");   //test 3
			...
			if ( level != 0 &&  level != 1)
			{	
				sms_err ("Bad value (%d)" , level);
				return -EINVAL;
			}
			gpio_set_value(host_sms_reset_pin, level);
		}
	default:
		return smschar_ioctl(file, cmd, arg);
	}
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = smschar_compat_ioctl,
#endif /* CONFIG_COMPAT */
};
...
int smschar_register(void)
{
	...
	kmutex_init(&g_smschar_socketslock);
	...
	/* socket 0*/
	atomic_set(&g_smschar_sockets[0].count, 0);
	for (i = 0; i < SMSCHAR_NR_NODES ; i++)
	{
	...
	for (j = 1; j<SMSCHAR_NR_SOCKETS ; j++)
	{
		...
		atomic_set(&g_smschar_sockets[j].count, 0);
...
void smschar_unregister(void)
{
	int i, j, devno;
	if (host_sms_reset_pin != 0xFFFFFFFF) {
		gpio_direction_output(host_sms_reset_pin, 0);
