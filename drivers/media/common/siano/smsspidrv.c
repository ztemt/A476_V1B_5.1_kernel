...
#ifdef SPI_WORKER_QUEUE
	struct task_struct	*spi_thread;
	atomic_t		spi_thread_abort;
...
/******************************************/
#ifdef SPI_WORKER_QUEUE
static void spi_worker_thread(struct _spi_device_st *spi_device)
{
#else
static void spi_worker_thread(struct work_struct *arg)
{
	...
	int is_list_empty;
	kmutex_lock(&spi_device->threadlock);
	do {
		/* do we have a msg to write ? */
		kmutex_lock(&spi_device->tx_queue_lock);
		...
		kmutex_unlock(&spi_device->tx_queue_lock);
		if (msg) {
			if (msg->add_preamble)
			{
				txmsg.len =
...
#ifdef SPI_WORKER_QUEUE
			if (atomic_read(&spi_device->irq_pending)){
				atomic_dec(&spi_device->irq_pending);
			}
#endif
		}
		...
		kmutex_lock(&spi_device->tx_queue_lock);
		is_list_empty = list_empty(&spi_device->txqueue);
		kmutex_unlock(&spi_device->tx_queue_lock);
	} while (!is_list_empty || msg);
	kmutex_unlock(&spi_device->threadlock);
...
//	sms_info("entering\n");
	...
	if (spi_device->wait_for_version_resp)
	{
		...
		{ /*This was an internal command, so we won't send it out*/
			...
			spi_device->chip_metal = ((u8*)(phdr+1))[3];
			sms_info("chip step=0x%x\n", spi_device->chip_metal);
...
#ifdef SPI_LOOPBACK_TEST
	if (phdr->msgType == MSG_SMS_LOOPBACK_RES)
	{ /*This was an internal command, so we won't send it out*/
		sms_info("loopback size: %d\n", min(spi_device->loopbacksize, (int)(phdr->msgLength - sizeof(*phdr))));
		
		if (spi_device->loopbacksize && spi_device->loopbackbuf) {
			memcpy(spi_device->loopbackbuf, (char *)(phdr+1), \
				min(spi_device->loopbacksize, (int)(phdr->msgLength - sizeof(*phdr))));
		}
		complete(&spi_device->loopback_done);
		free_rx_buf(spi_device, buf);
		return;
	}
#endif
...
static int smsspi_thread(void *device)
{
	struct _spi_device_st *spi_device = device;
	...
	do {
retry:
		ret = atomic_read(&spi_device->spi_thread_abort);
		if (ret)
			break;

//		sms_debug("irq_pending: %d\n", atomic_read(&spi_device->irq_pending));
//		sms_debug("[b]j: %lu, ms: %u\n", (jiffies - spi_device->irq_j), jiffies_to_msecs(jiffies - spi_device->irq_j));
		if (sms_chip_get_exists())
			spi_worker_thread(spi_device);
//		sms_debug("[a]j: %lu, ms: %u\n", (jiffies - spi_device->irq_j), jiffies_to_msecs(jiffies - spi_device->irq_j));
		kmutex_lock(&spi_device->tx_queue_lock);
...
//			sms_debug("irq_pending: %d\n", atomic_read(&spi_device->irq_pending));
		...
		if(!list_empty(&spi_device->txqueue)) {
			set_current_state(TASK_RUNNING);
			kmutex_unlock(&spi_device->tx_queue_lock);
//			sms_debug("txqueue...\n");
			goto retry;
...
//		sms_debug("wakeup.\n");
	...
	sms_debug("smsspi thread exiting with code %d\n", ret);

 return ret;
}

static int smsspi_thread_get(struct _spi_device_st *spi_device)
{
	 atomic_set(&spi_device->spi_thread_abort, 0);
	 atomic_set(&spi_device->irq_pending, 0);
	 spi_device->spi_thread =
		 kthread_run(smsspi_thread, spi_device, "smsspi");
	 if (IS_ERR(spi_device->spi_thread)) {
		 int err = PTR_ERR(spi_device->spi_thread);
		 spi_device->spi_thread = NULL;
		 return err;
	 }
...
static int smsspi_thread_put(struct _spi_device_st *spi_device)
{
	if (spi_device->spi_thread) {
		atomic_set(&spi_device->spi_thread_abort, 1);
		kthread_stop(spi_device->spi_thread);
		spi_device->spi_thread = NULL;
	}
...
static inline void smsspi_signal_thread(struct _spi_device_st *spi_device, bool is_irq_context)
{
	if (is_irq_context)
		atomic_inc(&spi_device->irq_pending);
	wake_up_process(spi_device->spi_thread);
}
#endif
...
#ifdef SPI_WORKER_QUEUE
	smsspi_signal_thread(spi_device, 0);
#else
...
static int smsspi_init_device(struct _spi_device_st *spi_device)
{
	...
	switch(spi_device->chip_model)
	{
		...
		case 0x4430: params.device_type = SMS_MANILA; break;
		case 0x4470: 
			if (spi_device->chip_metal >= 2)
...
#else
		if (!smsspiphy_is_device_exists() || kthread_should_stop()) {
			sms_info("Device is out. deinit\n");
			smsspi_deinit_device(spi_device);
			break;
		}
...
static int smsspi_bus_config(struct _spi_device_st *spi_device)
{
	int ret = 1;
	struct _smsspi_txmsg msg;
	
	struct SmsMsgData4Args_S setIntMsg = {{MSG_SMS_SPI_SET_BUS_WIDTH_REQ, 
		0, 
		11,
		sizeof(setIntMsg),
		0},
	...
	msg.buffer = &setIntMsg;
	msg.size = sizeof(setIntMsg);
	msg.prewrite = NULL;
	msg.postwrite = NULL;
	msg.alignment = SPI_PACKET_SIZE;
	msg.add_preamble = 1;
	ret = smsspi_queue_message_and_wait(spi_device, &msg);
	if (ret) {
		sms_err("Set INT line error, rc = %d\n", ret);
	}
	msleep(50);
	return ret;
}
...
#ifdef SPI_LOOPBACK_TEST
static int smsspi_bus_lookback_onepackage(struct _spi_device_st *spi_device, char *tx, int len, char *rx)
{
	int ret = 1;
	struct SmsMsgHdr_S smsmsg;
	struct _smsspi_txmsg msg;
	...
	if (!tx || !len || !rx)
		return 0;
#define PACKET_SIZE SPI_PACKET_SIZE

	init_completion(&spi_device->loopback_done);
	SMS_INIT_MSG(&smsmsg, MSG_SMS_LOOPBACK_REQ,
		     0);
	
	buff = kzalloc(PACKET_SIZE, GFP_KERNEL);
	if (!buff) {
		return 0;
		sms_err("No enough mem.\n");
	}

	spi_device->loopbackbuf = tx;
	spi_device->loopbacksize = size;
	size = min(len, (PACKET_SIZE - sizeof(smsmsg)));
	
	smsmsg.msgLength = size + sizeof(smsmsg);
	
	memcpy(buff, &smsmsg, sizeof(smsmsg));
	memcpy(buff + sizeof(smsmsg), tx, size);
	...
	/*Wait for response*/
	ret = wait_for_completion_timeout(&spi_device->loopback_done, msecs_to_jiffies(500));
	if (ret > 0)
	{
	...
	{ 
		sms_err("No response to loopback.\n");
	}
	spi_device->loopbackbuf = NULL;
	spi_device->loopbacksize = 0;
	return size;
}

static int smsspi_bus_lookback(_spi_device_st *spi_device)
{
	static char txdata[] = {0xaa, 0x55, 0xa5, 0x5a, 0x00, 0xff, 0xa0, 0x0a, 0xaf, 0xfa, 0x5f, 0xf5};
	static char rxdata[sizeof(txdata)];
	int times = 0;
	int i = 0
	while (++times <= 100) {
		smsspi_bus_lookback_onepackage(spi_device, txdata, sizeof(txdata), rxdata);
		if (0 != memcmp(txdata, rxdata, sizeof(txdata))) {
			sms_err("loopback errp:\n");
			sms_info("*****************************\n");
			sms_info("\t tx:\n");
			for (i = 0; i < sizeof(txdata); i++) {
				sms_info("%02x ", txdata[i]);
			}
			sms_info("*****************************\n");
			sms_info("\t rx:\n");
			for (i = 0; i < sizeof(txdata); i++) {
				sms_info("%02x ", rxdata[i]);
			}
		}
	}
...
static void smsspi_connect(struct work_struct *arg)
{
	...
	if (spi_device->device_initialized) {
		...
		fwDnlComplete(spi_device->phy_dev, 0);
#endif
		//smsspi_bus_config(spi_device);
#ifdef SPI_LOOPBACK_TEST
		smsspi_bus_lookback(spi_device)
#endif
	}
...
#ifdef SPI_WORKER_QUEUE
	spi_device->irq_j = jiffies;
	smsspi_signal_thread(spi_device, 1);
#else
	schedule_work(&spi_device->work_thread);
#endif
	}
...
int smsspi_plug_notify(int plugin)
{
	struct _spi_device_st *spi_device = spi_dev;
#if 0
	if (plugin == sms_chip_get_exists()) {
		sms_debug("%d it's already %s, return", __LINE__, plugin?"ON":"OFF");
		return 0;
	}
#endif
	sms_chip_set_exists(plugin);
	...
	if (plugin) {
		...
		if (i <= 0)
			sms_err("spi device plug out error!\n");
#else
		if (spi_device->thread != NULL) {
...
int smsspi_register(void)
{
	...
	spi_dev = spi_device;
	kmutex_init(&spi_device->threadlock);
	kmutex_init(&spi_device->tx_queue_lock);

	kmutex_lock(&spi_device->tx_queue_lock);
	INIT_LIST_HEAD(&spi_device->txqueue);
	kmutex_unlock(&spi_device->tx_queue_lock);
#ifndef SPI_WORKER_QUEUE
...
#ifdef SPI_WORKER_QUEUE
	ret = smsspi_thread_get(spi_device);
	if (ret) {
		sms_err("smsspi_thread_get failed: %d.\n", ret);
		goto txbuf_error;
	}
#endif
	spi_device->txbuf =
	    dma_alloc_coherent(siano_mm, TX_BUFFER_SIZE,
	...
	sms_debug("dma_alloc_coherent(txbuf): %p, size:%d\n", spi_device->txbuf, TX_BUFFER_SIZE);
	spi_device->cur_buf = 0;
	spi_device->private_buf[0].ptr = dma_alloc_coherent(siano_mm,
			RX_BUFFER_SIZE,
	...
	spi_device->private_buf[1].ptr = dma_alloc_coherent(siano_mm, 
			RX_BUFFER_SIZE,
